...
const char** q_packagekit__daemon_mime_types() {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_mime_types");
        abort();
    }
...
void q_packagekit__daemon_set_hints(const char* hints[]) {
    ...
    if (hints_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_set_hints");
        abort();
    }
...
const char** q_packagekit__daemon_hints() {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_hints");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
PackageKit__Transaction* q_packagekit__daemon_download_packages(const char* packageIDs[]) {
    ...
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_download_packages");
        abort();
    }
...
PackageKit__Transaction* q_packagekit__daemon_depends_on(const char* packageIDs[]) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_depends_on");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_get_details(const char* packageIDs[]) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_get_details");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_get_details_local(const char* files[]) {
    ...
    if (files_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_get_details_local");
        abort();
    }
...
PackageKit__Transaction* q_packagekit__daemon_get_files(const char* packageIDs[]) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_get_files");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_get_files_local(const char* files[]) {
    ...
    libqt_string* files_qstr = (libqt_string*)malloc(files_len * sizeof(libqt_string));
    if (files_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_get_files_local");
        abort();
    }
    for (size_t i = 0; i < files_len; ++i) {
        files_qstr[i] = qstring(files[i]);
...
PackageKit__Transaction* q_packagekit__daemon_required_by(const char* packageIDs[]) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_required_by");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_get_updates_details(const char* packageIDs[]) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_get_updates_details");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_install_files(const char* files[]) {
    ...
    libqt_string* files_qstr = (libqt_string*)malloc(files_len * sizeof(libqt_string));
    if (files_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_install_files");
        abort();
    }
    for (size_t i = 0; i < files_len; ++i) {
        files_qstr[i] = qstring(files[i]);
...
PackageKit__Transaction* q_packagekit__daemon_install_packages(const char* packageIDs[]) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_install_packages");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_remove_packages(const char* packageIDs[]) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_remove_packages");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_resolve(const char* packageNames[]) {
    ...
    if (packageNames_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_resolve");
        abort();
    }
...
PackageKit__Transaction* q_packagekit__daemon_search_files(const char* search[]) {
    ...
    if (search_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_search_files");
        abort();
    }
...
PackageKit__Transaction* q_packagekit__daemon_search_details(const char* search[]) {
    ...
    libqt_string* search_qstr = (libqt_string*)malloc(search_len * sizeof(libqt_string));
    if (search_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_search_details");
        abort();
    }
    for (size_t i = 0; i < search_len; ++i) {
        search_qstr[i] = qstring(search[i]);
...
PackageKit__Transaction* q_packagekit__daemon_search_groups(const char* groups[]) {
    ...
    if (groups_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_search_groups");
        abort();
    }
...
PackageKit__Transaction* q_packagekit__daemon_search_names(const char* search[]) {
    ...
    libqt_string* search_qstr = (libqt_string*)malloc(search_len * sizeof(libqt_string));
    if (search_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_search_names");
        abort();
    }
    for (size_t i = 0; i < search_len; ++i) {
        search_qstr[i] = qstring(search[i]);
...
PackageKit__Transaction* q_packagekit__daemon_update_packages(const char* packageIDs[]) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_update_packages");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_what_provides(const char* search[]) {
    ...
    libqt_string* search_qstr = (libqt_string*)malloc(search_len * sizeof(libqt_string));
    if (search_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_what_provides");
        abort();
    }
    for (size_t i = 0; i < search_len; ++i) {
        search_qstr[i] = qstring(search[i]);
...
void q_packagekit__daemon_transaction_list_changed(void* self, const char* tids[]) {
    ...
    if (tids_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_transaction_list_changed");
        abort();
    }
...
PackageKit__Transaction* q_packagekit__daemon_download_packages2(const char* packageIDs[], bool storeInCache) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_download_packages2");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_depends_on22(const char* packageIDs[], int32_t filters) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_depends_on22");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_depends_on3(const char* packageIDs[], int32_t filters, bool recursive) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_depends_on3");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_required_by22(const char* packageIDs[], int32_t filters) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_required_by22");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_required_by3(const char* packageIDs[], int32_t filters, bool recursive) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_required_by3");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_install_files2(const char* files[], int32_t flags) {
    ...
    libqt_string* files_qstr = (libqt_string*)malloc(files_len * sizeof(libqt_string));
    if (files_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_install_files2");
        abort();
    }
    for (size_t i = 0; i < files_len; ++i) {
        files_qstr[i] = qstring(files[i]);
...
PackageKit__Transaction* q_packagekit__daemon_install_packages2(const char* packageIDs[], int32_t flags) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_install_packages2");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_remove_packages2(const char* packageIDs[], bool allowDeps) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_remove_packages2");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_remove_packages3(const char* packageIDs[], bool allowDeps, bool autoRemove) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_remove_packages3");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_remove_packages4(const char* packageIDs[], bool allowDeps, bool autoRemove, int32_t flags) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_remove_packages4");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_resolve22(const char* packageNames[], int32_t filters) {
    ...
    libqt_string* packageNames_qstr = (libqt_string*)malloc(packageNames_len * sizeof(libqt_string));
    if (packageNames_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_resolve22");
        abort();
    }
    for (size_t i = 0; i < packageNames_len; ++i) {
        packageNames_qstr[i] = qstring(packageNames[i]);
...
PackageKit__Transaction* q_packagekit__daemon_search_files22(const char* search[], int32_t filters) {
    ...
    libqt_string* search_qstr = (libqt_string*)malloc(search_len * sizeof(libqt_string));
    if (search_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_search_files22");
        abort();
    }
    for (size_t i = 0; i < search_len; ++i) {
        search_qstr[i] = qstring(search[i]);
...
PackageKit__Transaction* q_packagekit__daemon_search_details22(const char* search[], int32_t filters) {
    ...
    libqt_string* search_qstr = (libqt_string*)malloc(search_len * sizeof(libqt_string));
    if (search_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_search_details22");
        abort();
    }
    for (size_t i = 0; i < search_len; ++i) {
        search_qstr[i] = qstring(search[i]);
...
PackageKit__Transaction* q_packagekit__daemon_search_groups22(const char* groups[], int32_t filters) {
    ...
    libqt_string* groups_qstr = (libqt_string*)malloc(groups_len * sizeof(libqt_string));
    if (groups_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_search_groups22");
        abort();
    }
    for (size_t i = 0; i < groups_len; ++i) {
        groups_qstr[i] = qstring(groups[i]);
...
PackageKit__Transaction* q_packagekit__daemon_search_names22(const char* search[], int32_t filters) {
    ...
    libqt_string* search_qstr = (libqt_string*)malloc(search_len * sizeof(libqt_string));
    if (search_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_search_names22");
        abort();
    }
    for (size_t i = 0; i < search_len; ++i) {
        search_qstr[i] = qstring(search[i]);
...
PackageKit__Transaction* q_packagekit__daemon_update_packages2(const char* packageIDs[], int32_t flags) {
    ...
    libqt_string* packageIDs_qstr = (libqt_string*)malloc(packageIDs_len * sizeof(libqt_string));
    if (packageIDs_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_update_packages2");
        abort();
    }
    for (size_t i = 0; i < packageIDs_len; ++i) {
        packageIDs_qstr[i] = qstring(packageIDs[i]);
...
PackageKit__Transaction* q_packagekit__daemon_what_provides22(const char* search[], int32_t filters) {
    ...
    libqt_string* search_qstr = (libqt_string*)malloc(search_len * sizeof(libqt_string));
    if (search_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_what_provides22");
        abort();
    }
    for (size_t i = 0; i < search_len; ++i) {
        search_qstr[i] = qstring(search[i]);
...
const char** q_packagekit__daemon_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_packagekit__daemon_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
