  ...
  public:
    ...
    PackageKit::Transaction::InternalError parseError(const QString& errorName) {
        if (packagekit__transaction_parseerror_isbase) {
            ...
            char* errorName_str = static_cast<char*>(malloc(errorName_str_len + 1));
            memcpy(errorName_str, errorName_b.data(), errorName_str_len);
            errorName_str[errorName_str_len] = '\0';
