...
const char* k_authorized_tr(const char* s) {
    libqt_string _str = KAuthorized_Tr(s);
...
const char* k_authorized_tr2(const char* s, const char* c) {
    libqt_string _str = KAuthorized_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_authorized_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KAuthorized_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** k_authorized_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_authorized_dynamic_property_names");
        abort();
    }
