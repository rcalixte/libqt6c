...
void k_config_add_config_sources(void* self, const char* sources[]) {
    ...
    if (sources_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_config_add_config_sources");
        abort();
    }
...
const char** k_config_additional_config_sources(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_config_additional_config_sources");
        abort();
    }
...
const char** k_config_group_list(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_config_group_list");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
