...
const char** k_configloader_group_list(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_configloader_group_list\n");
        abort();
    }
...
KCoreConfigSkeleton__ItemStringList* k_configloader_add_item_string_list(void* self, const char* name, const char* reference[static 1]) {
    ...
    if (reference_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_configloader_add_item_string_list\n");
        abort();
    }
...
KCoreConfigSkeleton__ItemStringList* k_configloader_add_item_string_list3(void* self, const char* name, const char* reference[static 1], const char* defaultValue[static 1]) {
    ...
    libqt_string* reference_qstr = (libqt_string*)malloc(reference_len * sizeof(libqt_string));
    if (reference_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_configloader_add_item_string_list3\n");
        abort();
    }
    for (size_t i = 0; i < reference_len; ++i) {
        reference_qstr[i] = qstring(reference[i]);
...
KCoreConfigSkeleton__ItemStringList* k_configloader_add_item_string_list4(void* self, const char* name, const char* reference[static 1], const char* defaultValue[static 1], const char* key) {
    ...
    libqt_string* reference_qstr = (libqt_string*)malloc(reference_len * sizeof(libqt_string));
    if (reference_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_configloader_add_item_string_list4\n");
        abort();
    }
    for (size_t i = 0; i < reference_len; ++i) {
        reference_qstr[i] = qstring(reference[i]);
...
void k_configloader_set_object_name(void* self, const char* name) {
...
int32_t k_configloader_start_timer2(void* self, int64_t time) {
...
const char** k_configloader_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_configloader_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
int32_t k_configloader_start_timer23(void* self, int64_t time, int32_t timerType) {
