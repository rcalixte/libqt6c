...
void QOpenGLFunctions_GlVertexAttribPointer(QOpenGLFunctions* self, uint32_t indx, int32_t size, uint32_t type, unsigned char normalized, int32_t stride, const void* ptr) {
    self->glVertexAttribPointer(static_cast<GLuint>(indx), static_cast<GLint>(size), static_cast<GLenum>(type), static_cast<GLboolean>(normalized), static_cast<GLsizei>(stride), ptr);
}
