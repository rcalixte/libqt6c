...
void q_placeicon_set_parameters(void* self, libqt_map /* of const char* to QVariant* */ parameters) {
    ...
    if (parameters_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in q_placeicon_set_parameters\n");
        abort();
    }
    ...
    if (parameters_ret.values == NULL) {
        ...
        fprintf(stderr, "Failed to allocate memory for map values in q_placeicon_set_parameters\n");
        abort();
    }
