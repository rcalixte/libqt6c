...
void QPlace_SetCategories(QPlace* self, const libqt_list /* of QPlaceCategory* */ categories) {
    ...
    QPlaceCategory** categories_arr = static_cast<QPlaceCategory**>(categories.data.ptr);
...
libqt_map /* of int to QPlaceContent* */ QPlace_Content(const QPlace* self, int type) {
    QMap<int, QPlaceContent> _ret = self->content(static_cast<QPlaceContent::Type>(type));
...
void QPlace_SetContent(QPlace* self, int type, const libqt_map /* of int to QPlaceContent* */ content) {
    ...
    self->setContent(static_cast<QPlaceContent::Type>(type), content_QMap);
}

void QPlace_InsertContent(QPlace* self, int type, const libqt_map /* of int to QPlaceContent* */ content) {
    QMap<int, QPlaceContent> content_QMap;
    int* content_karr = static_cast<int*>(content.keys);
    QPlaceContent** content_varr = static_cast<QPlaceContent**>(content.values);
    for (size_t i = 0; i < content.len; ++i) {
    ...
    self->insertContent(static_cast<QPlaceContent::Type>(type), content_QMap);
}

int QPlace_TotalContentCount(const QPlace* self, int type) {
    return self->totalContentCount(static_cast<QPlaceContent::Type>(type));
}

void QPlace_SetTotalContentCount(QPlace* self, int type, int total) {
    self->setTotalContentCount(static_cast<QPlaceContent::Type>(type), static_cast<int>(total));
}
...
void QPlace_SetContactDetails(QPlace* self, const libqt_string contactType, libqt_list /* of QPlaceContactDetail* */ details) {
    ...
    QPlaceContactDetail** details_arr = static_cast<QPlaceContactDetail**>(details.data.ptr);
