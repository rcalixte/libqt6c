...
const char** q_placesearchsuggestionreply_suggestions(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_placesearchsuggestionreply_suggestions");
        abort();
    }
...
void q_placesearchsuggestionreply_set_suggestions(void* self, const char* suggestions[static 1]) {
    ...
    if (suggestions_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_placesearchsuggestionreply_set_suggestions");
        abort();
    }
...
const char** q_placesearchsuggestionreply_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_placesearchsuggestionreply_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
