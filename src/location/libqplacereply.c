#include "../libqcoreevent.hpp"
#include "../libqmetaobject.hpp"
#include "../libqobjectdefs.hpp"
#include "../libqobject.hpp"
#include "libqplacereply.hpp"
#include "libqplacereply.h"

QPlaceReply* q_placereply_new() {
    return QPlaceReply_new();
}

QPlaceReply* q_placereply_new2(void* parent) {
    return QPlaceReply_new2((QObject*)parent);
}

const QMetaObject* q_placereply_meta_object(void* self) {
    return QPlaceReply_MetaObject((QPlaceReply*)self);
}

void q_placereply_on_meta_object(void* self, const QMetaObject* (*callback)()) {
    QPlaceReply_OnMetaObject((QPlaceReply*)self, (intptr_t)callback);
}

const QMetaObject* q_placereply_super_meta_object(void* self) {
    return QPlaceReply_SuperMetaObject((QPlaceReply*)self);
}

void* q_placereply_metacast(void* self, const char* param1) {
    return QPlaceReply_Metacast((QPlaceReply*)self, param1);
}

void q_placereply_on_metacast(void* self, void* (*callback)(void*, const char*)) {
    QPlaceReply_OnMetacast((QPlaceReply*)self, (intptr_t)callback);
}

void* q_placereply_super_metacast(void* self, const char* param1) {
    return QPlaceReply_SuperMetacast((QPlaceReply*)self, param1);
}

int32_t q_placereply_metacall(void* self, int32_t param1, int param2, void* param3) {
    return QPlaceReply_Metacall((QPlaceReply*)self, param1, param2, param3);
}

void q_placereply_on_metacall(void* self, int32_t (*callback)(void*, int32_t, int, void*)) {
    QPlaceReply_OnMetacall((QPlaceReply*)self, (intptr_t)callback);
}

int32_t q_placereply_super_metacall(void* self, int32_t param1, int param2, void* param3) {
    return QPlaceReply_SuperMetacall((QPlaceReply*)self, param1, param2, param3);
}

const char* q_placereply_tr(const char* s) {
    libqt_string _str = QObject_Tr(s);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

bool q_placereply_is_finished(void* self) {
    return QPlaceReply_IsFinished((QPlaceReply*)self);
}

int32_t q_placereply_type(void* self) {
    return QPlaceReply_Type((QPlaceReply*)self);
}

void q_placereply_on_type(void* self, int32_t (*callback)()) {
    QPlaceReply_OnType((QPlaceReply*)self, (intptr_t)callback);
}

int32_t q_placereply_super_type(void* self) {
    return QPlaceReply_SuperType((QPlaceReply*)self);
}

const char* q_placereply_error_string(void* self) {
    libqt_string _str = QPlaceReply_ErrorString((QPlaceReply*)self);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

int32_t q_placereply_error(void* self) {
    return QPlaceReply_Error((QPlaceReply*)self);
}

void q_placereply_abort(void* self) {
    QPlaceReply_Abort((QPlaceReply*)self);
}

void q_placereply_on_abort(void* self, void (*callback)()) {
    QPlaceReply_OnAbort((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_super_abort(void* self) {
    QPlaceReply_SuperAbort((QPlaceReply*)self);
}

void q_placereply_finished(void* self) {
    QPlaceReply_Finished((QPlaceReply*)self);
}

void q_placereply_on_finished(void* self, void (*callback)(void*)) {
    QPlaceReply_Connect_Finished((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_content_updated(void* self) {
    QPlaceReply_ContentUpdated((QPlaceReply*)self);
}

void q_placereply_on_content_updated(void* self, void (*callback)(void*)) {
    QPlaceReply_Connect_ContentUpdated((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_aborted(void* self) {
    QPlaceReply_Aborted((QPlaceReply*)self);
}

void q_placereply_on_aborted(void* self, void (*callback)(void*)) {
    QPlaceReply_Connect_Aborted((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_error_occurred(void* self, int32_t error) {
    QPlaceReply_ErrorOccurred((QPlaceReply*)self, error);
}

void q_placereply_on_error_occurred(void* self, void (*callback)(void*, int32_t)) {
    QPlaceReply_Connect_ErrorOccurred((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_set_finished(void* self, bool finished) {
    QPlaceReply_SetFinished((QPlaceReply*)self, finished);
}

void q_placereply_on_set_finished(void* self, void (*callback)(void*, bool)) {
    QPlaceReply_OnSetFinished((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_super_set_finished(void* self, bool finished) {
    QPlaceReply_SuperSetFinished((QPlaceReply*)self, finished);
}

void q_placereply_set_error(void* self, int32_t error, const char* errorString) {
    QPlaceReply_SetError((QPlaceReply*)self, error, qstring(errorString));
}

void q_placereply_on_set_error(void* self, void (*callback)(void*, int32_t, const char*)) {
    QPlaceReply_OnSetError((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_super_set_error(void* self, int32_t error, const char* errorString) {
    QPlaceReply_SuperSetError((QPlaceReply*)self, error, qstring(errorString));
}

const char* q_placereply_tr2(const char* s, const char* c) {
    libqt_string _str = QObject_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

const char* q_placereply_tr3(const char* s, const char* c, int n) {
    libqt_string _str = QObject_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void q_placereply_error_occurred2(void* self, int32_t error, const char* errorString) {
    QPlaceReply_ErrorOccurred2((QPlaceReply*)self, error, qstring(errorString));
}

void q_placereply_on_error_occurred2(void* self, void (*callback)(void*, int32_t, const char*)) {
    QPlaceReply_Connect_ErrorOccurred2((QPlaceReply*)self, (intptr_t)callback);
}

const char* q_placereply_object_name(void* self) {
    libqt_string _str = QObject_ObjectName((QObject*)self);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void q_placereply_set_object_name(void* self, const char* name) {
    QObject_SetObjectName((QObject*)self, name);
}

bool q_placereply_is_widget_type(void* self) {
    return QObject_IsWidgetType((QObject*)self);
}

bool q_placereply_is_window_type(void* self) {
    return QObject_IsWindowType((QObject*)self);
}

bool q_placereply_is_quick_item_type(void* self) {
    return QObject_IsQuickItemType((QObject*)self);
}

bool q_placereply_signals_blocked(void* self) {
    return QObject_SignalsBlocked((QObject*)self);
}

bool q_placereply_block_signals(void* self, bool b) {
    return QObject_BlockSignals((QObject*)self, b);
}

QThread* q_placereply_thread(void* self) {
    return QObject_Thread((QObject*)self);
}

bool q_placereply_move_to_thread(void* self, void* thread) {
    return QObject_MoveToThread((QObject*)self, (QThread*)thread);
}

int32_t q_placereply_start_timer(void* self, int interval) {
    return QObject_StartTimer((QObject*)self, interval);
}

int32_t q_placereply_start_timer2(void* self, int64_t time) {
    return QObject_StartTimer2((QObject*)self, time);
}

void q_placereply_kill_timer(void* self, int id) {
    QObject_KillTimer((QObject*)self, id);
}

void q_placereply_kill_timer2(void* self, int32_t id) {
    QObject_KillTimer2((QObject*)self, id);
}

libqt_list /* of QObject* */ q_placereply_children(void* self) {
    libqt_list _arr = QObject_Children((QObject*)self);
    return _arr;
}

void q_placereply_set_parent(void* self, void* parent) {
    QObject_SetParent((QObject*)self, (QObject*)parent);
}

void q_placereply_install_event_filter(void* self, void* filterObj) {
    QObject_InstallEventFilter((QObject*)self, (QObject*)filterObj);
}

void q_placereply_remove_event_filter(void* self, void* obj) {
    QObject_RemoveEventFilter((QObject*)self, (QObject*)obj);
}

QMetaObject__Connection* q_placereply_connect(void* sender, const char* signal, void* receiver, const char* member) {
    return QObject_Connect((QObject*)sender, signal, (QObject*)receiver, member);
}

QMetaObject__Connection* q_placereply_connect2(void* sender, void* signal, void* receiver, void* method) {
    return QObject_Connect2((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method);
}

QMetaObject__Connection* q_placereply_connect3(void* self, void* sender, const char* signal, const char* member) {
    return QObject_Connect3((QObject*)self, (QObject*)sender, signal, member);
}

bool q_placereply_disconnect(void* sender, const char* signal, void* receiver, const char* member) {
    return QObject_Disconnect((QObject*)sender, signal, (QObject*)receiver, member);
}

bool q_placereply_disconnect2(void* sender, void* signal, void* receiver, void* member) {
    return QObject_Disconnect2((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)member);
}

bool q_placereply_disconnect3(void* self) {
    return QObject_Disconnect3((QObject*)self);
}

bool q_placereply_disconnect4(void* self, void* receiver) {
    return QObject_Disconnect4((QObject*)self, (QObject*)receiver);
}

bool q_placereply_disconnect5(void* param1) {
    return QObject_Disconnect5((QMetaObject__Connection*)param1);
}

void q_placereply_dump_object_tree(void* self) {
    QObject_DumpObjectTree((QObject*)self);
}

void q_placereply_dump_object_info(void* self) {
    QObject_DumpObjectInfo((QObject*)self);
}

bool q_placereply_set_property(void* self, const char* name, void* value) {
    return QObject_SetProperty((QObject*)self, name, (QVariant*)value);
}

QVariant* q_placereply_property(void* self, const char* name) {
    return QObject_Property((QObject*)self, name);
}

const char** q_placereply_dynamic_property_names(void* self) {
    libqt_list _arr = QObject_DynamicPropertyNames((QObject*)self);
    const libqt_string* _qstr = (libqt_string*)_arr.data.ptr;
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_placereply_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
    }
    _ret[_arr.len] = NULL;
    for (size_t i = 0; i < _arr.len; ++i) {
        libqt_string_free((libqt_string*)&_qstr[i]);
    }
    libqt_free(_arr.data.ptr);
    return _ret;
}

QBindingStorage* q_placereply_binding_storage(void* self) {
    return QObject_BindingStorage((QObject*)self);
}

const QBindingStorage* q_placereply_binding_storage2(void* self) {
    return QObject_BindingStorage2((QObject*)self);
}

void q_placereply_destroyed(void* self) {
    QObject_Destroyed((QObject*)self);
}

void q_placereply_on_destroyed(void* self, void (*callback)(void*)) {
    QObject_Connect_Destroyed((QObject*)self, (intptr_t)callback);
}

QObject* q_placereply_parent(void* self) {
    return QObject_Parent((QObject*)self);
}

bool q_placereply_inherits(void* self, const char* classname) {
    return QObject_Inherits((QObject*)self, classname);
}

void q_placereply_delete_later(void* self) {
    QObject_DeleteLater((QObject*)self);
}

int32_t q_placereply_start_timer22(void* self, int interval, int32_t timerType) {
    return QObject_StartTimer22((QObject*)self, interval, timerType);
}

int32_t q_placereply_start_timer23(void* self, int64_t time, int32_t timerType) {
    return QObject_StartTimer23((QObject*)self, time, timerType);
}

QMetaObject__Connection* q_placereply_connect5(void* sender, const char* signal, void* receiver, const char* member, int32_t param5) {
    return QObject_Connect5((QObject*)sender, signal, (QObject*)receiver, member, param5);
}

QMetaObject__Connection* q_placereply_connect52(void* sender, void* signal, void* receiver, void* method, int32_t type) {
    return QObject_Connect52((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method, type);
}

QMetaObject__Connection* q_placereply_connect4(void* self, void* sender, const char* signal, const char* member, int32_t type) {
    return QObject_Connect4((QObject*)self, (QObject*)sender, signal, member, type);
}

bool q_placereply_disconnect1(void* self, const char* signal) {
    return QObject_Disconnect1((QObject*)self, signal);
}

bool q_placereply_disconnect22(void* self, const char* signal, void* receiver) {
    return QObject_Disconnect22((QObject*)self, signal, (QObject*)receiver);
}

bool q_placereply_disconnect32(void* self, const char* signal, void* receiver, const char* member) {
    return QObject_Disconnect32((QObject*)self, signal, (QObject*)receiver, member);
}

bool q_placereply_disconnect23(void* self, void* receiver, const char* member) {
    return QObject_Disconnect23((QObject*)self, (QObject*)receiver, member);
}

void q_placereply_destroyed1(void* self, void* param1) {
    QObject_Destroyed1((QObject*)self, (QObject*)param1);
}

void q_placereply_on_destroyed1(void* self, void (*callback)(void*, void*)) {
    QObject_Connect_Destroyed1((QObject*)self, (intptr_t)callback);
}

bool q_placereply_event(void* self, void* event) {
    return QPlaceReply_Event((QPlaceReply*)self, (QEvent*)event);
}

bool q_placereply_super_event(void* self, void* event) {
    return QPlaceReply_SuperEvent((QPlaceReply*)self, (QEvent*)event);
}

void q_placereply_on_event(void* self, bool (*callback)(void*, void*)) {
    QPlaceReply_OnEvent((QPlaceReply*)self, (intptr_t)callback);
}

bool q_placereply_event_filter(void* self, void* watched, void* event) {
    return QPlaceReply_EventFilter((QPlaceReply*)self, (QObject*)watched, (QEvent*)event);
}

bool q_placereply_super_event_filter(void* self, void* watched, void* event) {
    return QPlaceReply_SuperEventFilter((QPlaceReply*)self, (QObject*)watched, (QEvent*)event);
}

void q_placereply_on_event_filter(void* self, bool (*callback)(void*, void*, void*)) {
    QPlaceReply_OnEventFilter((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_timer_event(void* self, void* event) {
    QPlaceReply_TimerEvent((QPlaceReply*)self, (QTimerEvent*)event);
}

void q_placereply_super_timer_event(void* self, void* event) {
    QPlaceReply_SuperTimerEvent((QPlaceReply*)self, (QTimerEvent*)event);
}

void q_placereply_on_timer_event(void* self, void (*callback)(void*, void*)) {
    QPlaceReply_OnTimerEvent((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_child_event(void* self, void* event) {
    QPlaceReply_ChildEvent((QPlaceReply*)self, (QChildEvent*)event);
}

void q_placereply_super_child_event(void* self, void* event) {
    QPlaceReply_SuperChildEvent((QPlaceReply*)self, (QChildEvent*)event);
}

void q_placereply_on_child_event(void* self, void (*callback)(void*, void*)) {
    QPlaceReply_OnChildEvent((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_custom_event(void* self, void* event) {
    QPlaceReply_CustomEvent((QPlaceReply*)self, (QEvent*)event);
}

void q_placereply_super_custom_event(void* self, void* event) {
    QPlaceReply_SuperCustomEvent((QPlaceReply*)self, (QEvent*)event);
}

void q_placereply_on_custom_event(void* self, void (*callback)(void*, void*)) {
    QPlaceReply_OnCustomEvent((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_connect_notify(void* self, void* signal) {
    QPlaceReply_ConnectNotify((QPlaceReply*)self, (QMetaMethod*)signal);
}

void q_placereply_super_connect_notify(void* self, void* signal) {
    QPlaceReply_SuperConnectNotify((QPlaceReply*)self, (QMetaMethod*)signal);
}

void q_placereply_on_connect_notify(void* self, void (*callback)(void*, void*)) {
    QPlaceReply_OnConnectNotify((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_disconnect_notify(void* self, void* signal) {
    QPlaceReply_DisconnectNotify((QPlaceReply*)self, (QMetaMethod*)signal);
}

void q_placereply_super_disconnect_notify(void* self, void* signal) {
    QPlaceReply_SuperDisconnectNotify((QPlaceReply*)self, (QMetaMethod*)signal);
}

void q_placereply_on_disconnect_notify(void* self, void (*callback)(void*, void*)) {
    QPlaceReply_OnDisconnectNotify((QPlaceReply*)self, (intptr_t)callback);
}

QObject* q_placereply_sender(void* self) {
    return QPlaceReply_Sender((QPlaceReply*)self);
}

QObject* q_placereply_super_sender(void* self) {
    return QPlaceReply_SuperSender((QPlaceReply*)self);
}

void q_placereply_on_sender(void* self, QObject* (*callback)()) {
    QPlaceReply_OnSender((QPlaceReply*)self, (intptr_t)callback);
}

int32_t q_placereply_sender_signal_index(void* self) {
    return QPlaceReply_SenderSignalIndex((QPlaceReply*)self);
}

int32_t q_placereply_super_sender_signal_index(void* self) {
    return QPlaceReply_SuperSenderSignalIndex((QPlaceReply*)self);
}

void q_placereply_on_sender_signal_index(void* self, int32_t (*callback)()) {
    QPlaceReply_OnSenderSignalIndex((QPlaceReply*)self, (intptr_t)callback);
}

int32_t q_placereply_receivers(void* self, const char* signal) {
    return QPlaceReply_Receivers((QPlaceReply*)self, signal);
}

int32_t q_placereply_super_receivers(void* self, const char* signal) {
    return QPlaceReply_SuperReceivers((QPlaceReply*)self, signal);
}

void q_placereply_on_receivers(void* self, int32_t (*callback)(void*, const char*)) {
    QPlaceReply_OnReceivers((QPlaceReply*)self, (intptr_t)callback);
}

bool q_placereply_is_signal_connected(void* self, void* signal) {
    return QPlaceReply_IsSignalConnected((QPlaceReply*)self, (QMetaMethod*)signal);
}

bool q_placereply_super_is_signal_connected(void* self, void* signal) {
    return QPlaceReply_SuperIsSignalConnected((QPlaceReply*)self, (QMetaMethod*)signal);
}

void q_placereply_on_is_signal_connected(void* self, bool (*callback)(void*, void*)) {
    QPlaceReply_OnIsSignalConnected((QPlaceReply*)self, (intptr_t)callback);
}

void q_placereply_on_object_name_changed(void* self, void (*callback)(void*, const char*)) {
    QObject_Connect_ObjectNameChanged((QObject*)self, (intptr_t)callback);
}

void q_placereply_delete(void* self) {
    QPlaceReply_Delete((QPlaceReply*)(self));
}
