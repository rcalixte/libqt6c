#include "../libqchar.hpp"
#include "../libqcoreevent.hpp"
#include "../libqcolor.hpp"
#include "../libqfont.hpp"
#include "../libqmetaobject.hpp"
#include "../libqobjectdefs.hpp"
#include "../libqobject.hpp"
#include "../libqsettings.hpp"
#include "libqscilexer.hpp"
#include "libqsciscintilla.hpp"
#include "libqscilexerasm.hpp"
#include "libqscilexerasm.h"

QsciLexerAsm* q_scilexerasm_new() {
    return QsciLexerAsm_new();
}

QsciLexerAsm* q_scilexerasm_new2(void* parent) {
    return QsciLexerAsm_new2((QObject*)parent);
}

const QMetaObject* q_scilexerasm_meta_object(void* self) {
    return QsciLexerAsm_MetaObject((QsciLexerAsm*)self);
}

void q_scilexerasm_on_meta_object(void* self, const QMetaObject* (*callback)()) {
    QsciLexerAsm_OnMetaObject((QsciLexerAsm*)self, (intptr_t)callback);
}

const QMetaObject* q_scilexerasm_super_meta_object(void* self) {
    return QsciLexerAsm_SuperMetaObject((QsciLexerAsm*)self);
}

void* q_scilexerasm_metacast(void* self, const char* param1) {
    return QsciLexerAsm_Metacast((QsciLexerAsm*)self, param1);
}

void q_scilexerasm_on_metacast(void* self, void* (*callback)(void*, const char*)) {
    QsciLexerAsm_OnMetacast((QsciLexerAsm*)self, (intptr_t)callback);
}

void* q_scilexerasm_super_metacast(void* self, const char* param1) {
    return QsciLexerAsm_SuperMetacast((QsciLexerAsm*)self, param1);
}

int32_t q_scilexerasm_metacall(void* self, int32_t param1, int param2, void* param3) {
    return QsciLexerAsm_Metacall((QsciLexerAsm*)self, param1, param2, param3);
}

void q_scilexerasm_on_metacall(void* self, int32_t (*callback)(void*, int32_t, int, void*)) {
    QsciLexerAsm_OnMetacall((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_super_metacall(void* self, int32_t param1, int param2, void* param3) {
    return QsciLexerAsm_SuperMetacall((QsciLexerAsm*)self, param1, param2, param3);
}

const char* q_scilexerasm_tr(const char* s) {
    libqt_string _str = QObject_Tr(s);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

QColor* q_scilexerasm_default_color(void* self, int style) {
    return QsciLexerAsm_DefaultColor((QsciLexerAsm*)self, style);
}

bool q_scilexerasm_default_eol_fill(void* self, int style) {
    return QsciLexerAsm_DefaultEolFill((QsciLexerAsm*)self, style);
}

QFont* q_scilexerasm_default_font(void* self, int style) {
    return QsciLexerAsm_DefaultFont((QsciLexerAsm*)self, style);
}

QColor* q_scilexerasm_default_paper(void* self, int style) {
    return QsciLexerAsm_DefaultPaper((QsciLexerAsm*)self, style);
}

const char* q_scilexerasm_keywords(void* self, int set) {
    return QsciLexerAsm_Keywords((QsciLexerAsm*)self, set);
}

const char* q_scilexerasm_description(void* self, int style) {
    libqt_string _str = QsciLexerAsm_Description((QsciLexerAsm*)self, style);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void q_scilexerasm_refresh_properties(void* self) {
    QsciLexerAsm_RefreshProperties((QsciLexerAsm*)self);
}

bool q_scilexerasm_fold_comments(void* self) {
    return QsciLexerAsm_FoldComments((QsciLexerAsm*)self);
}

bool q_scilexerasm_fold_compact(void* self) {
    return QsciLexerAsm_FoldCompact((QsciLexerAsm*)self);
}

QChar* q_scilexerasm_comment_delimiter(void* self) {
    return QsciLexerAsm_CommentDelimiter((QsciLexerAsm*)self);
}

bool q_scilexerasm_fold_syntax_based(void* self) {
    return QsciLexerAsm_FoldSyntaxBased((QsciLexerAsm*)self);
}

void q_scilexerasm_set_fold_comments(void* self, bool fold) {
    QsciLexerAsm_SetFoldComments((QsciLexerAsm*)self, fold);
}

void q_scilexerasm_on_set_fold_comments(void* self, void (*callback)(void*, bool)) {
    QsciLexerAsm_OnSetFoldComments((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_super_set_fold_comments(void* self, bool fold) {
    QsciLexerAsm_SuperSetFoldComments((QsciLexerAsm*)self, fold);
}

void q_scilexerasm_set_fold_compact(void* self, bool fold) {
    QsciLexerAsm_SetFoldCompact((QsciLexerAsm*)self, fold);
}

void q_scilexerasm_on_set_fold_compact(void* self, void (*callback)(void*, bool)) {
    QsciLexerAsm_OnSetFoldCompact((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_super_set_fold_compact(void* self, bool fold) {
    QsciLexerAsm_SuperSetFoldCompact((QsciLexerAsm*)self, fold);
}

void q_scilexerasm_set_comment_delimiter(void* self, void* delimeter) {
    QsciLexerAsm_SetCommentDelimiter((QsciLexerAsm*)self, (QChar*)delimeter);
}

void q_scilexerasm_on_set_comment_delimiter(void* self, void (*callback)(void*, void*)) {
    QsciLexerAsm_OnSetCommentDelimiter((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_super_set_comment_delimiter(void* self, void* delimeter) {
    QsciLexerAsm_SuperSetCommentDelimiter((QsciLexerAsm*)self, (QChar*)delimeter);
}

void q_scilexerasm_set_fold_syntax_based(void* self, bool syntax_based) {
    QsciLexerAsm_SetFoldSyntaxBased((QsciLexerAsm*)self, syntax_based);
}

void q_scilexerasm_on_set_fold_syntax_based(void* self, void (*callback)(void*, bool)) {
    QsciLexerAsm_OnSetFoldSyntaxBased((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_super_set_fold_syntax_based(void* self, bool syntax_based) {
    QsciLexerAsm_SuperSetFoldSyntaxBased((QsciLexerAsm*)self, syntax_based);
}

bool q_scilexerasm_read_properties(void* self, void* qs, const char* prefix) {
    return QsciLexerAsm_ReadProperties((QsciLexerAsm*)self, (QSettings*)qs, qstring(prefix));
}

void q_scilexerasm_on_read_properties(void* self, bool (*callback)(void*, void*, const char*)) {
    QsciLexerAsm_OnReadProperties((QsciLexerAsm*)self, (intptr_t)callback);
}

bool q_scilexerasm_super_read_properties(void* self, void* qs, const char* prefix) {
    return QsciLexerAsm_SuperReadProperties((QsciLexerAsm*)self, (QSettings*)qs, qstring(prefix));
}

bool q_scilexerasm_write_properties(void* self, void* qs, const char* prefix) {
    return QsciLexerAsm_WriteProperties((QsciLexerAsm*)self, (QSettings*)qs, qstring(prefix));
}

void q_scilexerasm_on_write_properties(void* self, bool (*callback)(void*, void*, const char*)) {
    QsciLexerAsm_OnWriteProperties((QsciLexerAsm*)self, (intptr_t)callback);
}

bool q_scilexerasm_super_write_properties(void* self, void* qs, const char* prefix) {
    return QsciLexerAsm_SuperWriteProperties((QsciLexerAsm*)self, (QSettings*)qs, qstring(prefix));
}

const char* q_scilexerasm_tr2(const char* s, const char* c) {
    libqt_string _str = QObject_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

const char* q_scilexerasm_tr3(const char* s, const char* c, int n) {
    libqt_string _str = QObject_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

QsciAbstractAPIs* q_scilexerasm_apis(void* self) {
    return QsciLexer_Apis((QsciLexer*)self);
}

int32_t q_scilexerasm_auto_indent_style(void* self) {
    return QsciLexer_AutoIndentStyle((QsciLexer*)self);
}

QsciScintilla* q_scilexerasm_editor(void* self) {
    return QsciLexer_Editor((QsciLexer*)self);
}

void q_scilexerasm_set_a_p_is(void* self, void* apis) {
    QsciLexer_SetAPIs((QsciLexer*)self, (QsciAbstractAPIs*)apis);
}

void q_scilexerasm_set_default_color(void* self, void* c) {
    QsciLexer_SetDefaultColor((QsciLexer*)self, (QColor*)c);
}

void q_scilexerasm_set_default_font(void* self, void* f) {
    QsciLexer_SetDefaultFont((QsciLexer*)self, (QFont*)f);
}

void q_scilexerasm_set_default_paper(void* self, void* c) {
    QsciLexer_SetDefaultPaper((QsciLexer*)self, (QColor*)c);
}

bool q_scilexerasm_read_settings(void* self, void* qs) {
    return QsciLexer_ReadSettings((QsciLexer*)self, (QSettings*)qs);
}

bool q_scilexerasm_write_settings(void* self, void* qs) {
    return QsciLexer_WriteSettings((QsciLexer*)self, (QSettings*)qs);
}

void q_scilexerasm_color_changed(void* self, void* c, int style) {
    QsciLexer_ColorChanged((QsciLexer*)self, (QColor*)c, style);
}

void q_scilexerasm_on_color_changed(void* self, void (*callback)(void*, void*, int)) {
    QsciLexer_Connect_ColorChanged((QsciLexer*)self, (intptr_t)callback);
}

void q_scilexerasm_eol_fill_changed(void* self, bool eolfilled, int style) {
    QsciLexer_EolFillChanged((QsciLexer*)self, eolfilled, style);
}

void q_scilexerasm_on_eol_fill_changed(void* self, void (*callback)(void*, bool, int)) {
    QsciLexer_Connect_EolFillChanged((QsciLexer*)self, (intptr_t)callback);
}

void q_scilexerasm_font_changed(void* self, void* f, int style) {
    QsciLexer_FontChanged((QsciLexer*)self, (QFont*)f, style);
}

void q_scilexerasm_on_font_changed(void* self, void (*callback)(void*, void*, int)) {
    QsciLexer_Connect_FontChanged((QsciLexer*)self, (intptr_t)callback);
}

void q_scilexerasm_paper_changed(void* self, void* c, int style) {
    QsciLexer_PaperChanged((QsciLexer*)self, (QColor*)c, style);
}

void q_scilexerasm_on_paper_changed(void* self, void (*callback)(void*, void*, int)) {
    QsciLexer_Connect_PaperChanged((QsciLexer*)self, (intptr_t)callback);
}

void q_scilexerasm_property_changed(void* self, const char* prop, const char* val) {
    QsciLexer_PropertyChanged((QsciLexer*)self, prop, val);
}

void q_scilexerasm_on_property_changed(void* self, void (*callback)(void*, const char*, const char*)) {
    QsciLexer_Connect_PropertyChanged((QsciLexer*)self, (intptr_t)callback);
}

bool q_scilexerasm_read_settings2(void* self, void* qs, const char* prefix) {
    return QsciLexer_ReadSettings2((QsciLexer*)self, (QSettings*)qs, prefix);
}

bool q_scilexerasm_write_settings2(void* self, void* qs, const char* prefix) {
    return QsciLexer_WriteSettings2((QsciLexer*)self, (QSettings*)qs, prefix);
}

const char* q_scilexerasm_object_name(void* self) {
    libqt_string _str = QObject_ObjectName((QObject*)self);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void q_scilexerasm_set_object_name(void* self, const char* name) {
    QObject_SetObjectName((QObject*)self, name);
}

bool q_scilexerasm_is_widget_type(void* self) {
    return QObject_IsWidgetType((QObject*)self);
}

bool q_scilexerasm_is_window_type(void* self) {
    return QObject_IsWindowType((QObject*)self);
}

bool q_scilexerasm_is_quick_item_type(void* self) {
    return QObject_IsQuickItemType((QObject*)self);
}

bool q_scilexerasm_signals_blocked(void* self) {
    return QObject_SignalsBlocked((QObject*)self);
}

bool q_scilexerasm_block_signals(void* self, bool b) {
    return QObject_BlockSignals((QObject*)self, b);
}

QThread* q_scilexerasm_thread(void* self) {
    return QObject_Thread((QObject*)self);
}

bool q_scilexerasm_move_to_thread(void* self, void* thread) {
    return QObject_MoveToThread((QObject*)self, (QThread*)thread);
}

int32_t q_scilexerasm_start_timer(void* self, int interval) {
    return QObject_StartTimer((QObject*)self, interval);
}

int32_t q_scilexerasm_start_timer2(void* self, int64_t time) {
    return QObject_StartTimer2((QObject*)self, time);
}

void q_scilexerasm_kill_timer(void* self, int id) {
    QObject_KillTimer((QObject*)self, id);
}

void q_scilexerasm_kill_timer2(void* self, int32_t id) {
    QObject_KillTimer2((QObject*)self, id);
}

libqt_list /* of QObject* */ q_scilexerasm_children(void* self) {
    libqt_list _arr = QObject_Children((QObject*)self);
    return _arr;
}

void q_scilexerasm_set_parent(void* self, void* parent) {
    QObject_SetParent((QObject*)self, (QObject*)parent);
}

void q_scilexerasm_install_event_filter(void* self, void* filterObj) {
    QObject_InstallEventFilter((QObject*)self, (QObject*)filterObj);
}

void q_scilexerasm_remove_event_filter(void* self, void* obj) {
    QObject_RemoveEventFilter((QObject*)self, (QObject*)obj);
}

QMetaObject__Connection* q_scilexerasm_connect(void* sender, const char* signal, void* receiver, const char* member) {
    return QObject_Connect((QObject*)sender, signal, (QObject*)receiver, member);
}

QMetaObject__Connection* q_scilexerasm_connect2(void* sender, void* signal, void* receiver, void* method) {
    return QObject_Connect2((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method);
}

QMetaObject__Connection* q_scilexerasm_connect3(void* self, void* sender, const char* signal, const char* member) {
    return QObject_Connect3((QObject*)self, (QObject*)sender, signal, member);
}

bool q_scilexerasm_disconnect(void* sender, const char* signal, void* receiver, const char* member) {
    return QObject_Disconnect((QObject*)sender, signal, (QObject*)receiver, member);
}

bool q_scilexerasm_disconnect2(void* sender, void* signal, void* receiver, void* member) {
    return QObject_Disconnect2((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)member);
}

bool q_scilexerasm_disconnect3(void* self) {
    return QObject_Disconnect3((QObject*)self);
}

bool q_scilexerasm_disconnect4(void* self, void* receiver) {
    return QObject_Disconnect4((QObject*)self, (QObject*)receiver);
}

bool q_scilexerasm_disconnect5(void* param1) {
    return QObject_Disconnect5((QMetaObject__Connection*)param1);
}

void q_scilexerasm_dump_object_tree(void* self) {
    QObject_DumpObjectTree((QObject*)self);
}

void q_scilexerasm_dump_object_info(void* self) {
    QObject_DumpObjectInfo((QObject*)self);
}

bool q_scilexerasm_set_property(void* self, const char* name, void* value) {
    return QObject_SetProperty((QObject*)self, name, (QVariant*)value);
}

QVariant* q_scilexerasm_property(void* self, const char* name) {
    return QObject_Property((QObject*)self, name);
}

const char** q_scilexerasm_dynamic_property_names(void* self) {
    libqt_list _arr = QObject_DynamicPropertyNames((QObject*)self);
    const libqt_string* _qstr = (libqt_string*)_arr.data.ptr;
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_scilexerasm_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
    }
    _ret[_arr.len] = NULL;
    for (size_t i = 0; i < _arr.len; ++i) {
        libqt_string_free((libqt_string*)&_qstr[i]);
    }
    libqt_free(_arr.data.ptr);
    return _ret;
}

QBindingStorage* q_scilexerasm_binding_storage(void* self) {
    return QObject_BindingStorage((QObject*)self);
}

const QBindingStorage* q_scilexerasm_binding_storage2(void* self) {
    return QObject_BindingStorage2((QObject*)self);
}

void q_scilexerasm_destroyed(void* self) {
    QObject_Destroyed((QObject*)self);
}

void q_scilexerasm_on_destroyed(void* self, void (*callback)(void*)) {
    QObject_Connect_Destroyed((QObject*)self, (intptr_t)callback);
}

QObject* q_scilexerasm_parent(void* self) {
    return QObject_Parent((QObject*)self);
}

bool q_scilexerasm_inherits(void* self, const char* classname) {
    return QObject_Inherits((QObject*)self, classname);
}

void q_scilexerasm_delete_later(void* self) {
    QObject_DeleteLater((QObject*)self);
}

int32_t q_scilexerasm_start_timer22(void* self, int interval, int32_t timerType) {
    return QObject_StartTimer22((QObject*)self, interval, timerType);
}

int32_t q_scilexerasm_start_timer23(void* self, int64_t time, int32_t timerType) {
    return QObject_StartTimer23((QObject*)self, time, timerType);
}

QMetaObject__Connection* q_scilexerasm_connect5(void* sender, const char* signal, void* receiver, const char* member, int32_t param5) {
    return QObject_Connect5((QObject*)sender, signal, (QObject*)receiver, member, param5);
}

QMetaObject__Connection* q_scilexerasm_connect52(void* sender, void* signal, void* receiver, void* method, int32_t type) {
    return QObject_Connect52((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method, type);
}

QMetaObject__Connection* q_scilexerasm_connect4(void* self, void* sender, const char* signal, const char* member, int32_t type) {
    return QObject_Connect4((QObject*)self, (QObject*)sender, signal, member, type);
}

bool q_scilexerasm_disconnect1(void* self, const char* signal) {
    return QObject_Disconnect1((QObject*)self, signal);
}

bool q_scilexerasm_disconnect22(void* self, const char* signal, void* receiver) {
    return QObject_Disconnect22((QObject*)self, signal, (QObject*)receiver);
}

bool q_scilexerasm_disconnect32(void* self, const char* signal, void* receiver, const char* member) {
    return QObject_Disconnect32((QObject*)self, signal, (QObject*)receiver, member);
}

bool q_scilexerasm_disconnect23(void* self, void* receiver, const char* member) {
    return QObject_Disconnect23((QObject*)self, (QObject*)receiver, member);
}

void q_scilexerasm_destroyed1(void* self, void* param1) {
    QObject_Destroyed1((QObject*)self, (QObject*)param1);
}

void q_scilexerasm_on_destroyed1(void* self, void (*callback)(void*, void*)) {
    QObject_Connect_Destroyed1((QObject*)self, (intptr_t)callback);
}

const char* q_scilexerasm_language(void* self) {
    return QsciLexerAsm_Language((QsciLexerAsm*)self);
}

const char* q_scilexerasm_super_language(void* self) {
    return QsciLexerAsm_SuperLanguage((QsciLexerAsm*)self);
}

void q_scilexerasm_on_language(void* self, const char* (*callback)()) {
    QsciLexerAsm_OnLanguage((QsciLexerAsm*)self, (intptr_t)callback);
}

const char* q_scilexerasm_lexer(void* self) {
    return QsciLexerAsm_Lexer((QsciLexerAsm*)self);
}

const char* q_scilexerasm_super_lexer(void* self) {
    return QsciLexerAsm_SuperLexer((QsciLexerAsm*)self);
}

void q_scilexerasm_on_lexer(void* self, const char* (*callback)()) {
    QsciLexerAsm_OnLexer((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_lexer_id(void* self) {
    return QsciLexerAsm_LexerId((QsciLexerAsm*)self);
}

int32_t q_scilexerasm_super_lexer_id(void* self) {
    return QsciLexerAsm_SuperLexerId((QsciLexerAsm*)self);
}

void q_scilexerasm_on_lexer_id(void* self, int32_t (*callback)()) {
    QsciLexerAsm_OnLexerId((QsciLexerAsm*)self, (intptr_t)callback);
}

const char* q_scilexerasm_auto_completion_fillups(void* self) {
    return QsciLexerAsm_AutoCompletionFillups((QsciLexerAsm*)self);
}

const char* q_scilexerasm_super_auto_completion_fillups(void* self) {
    return QsciLexerAsm_SuperAutoCompletionFillups((QsciLexerAsm*)self);
}

void q_scilexerasm_on_auto_completion_fillups(void* self, const char* (*callback)()) {
    QsciLexerAsm_OnAutoCompletionFillups((QsciLexerAsm*)self, (intptr_t)callback);
}

const char** q_scilexerasm_auto_completion_word_separators(void* self) {
    libqt_list _arr = QsciLexerAsm_AutoCompletionWordSeparators((QsciLexerAsm*)self);
    const libqt_string* _qstr = (libqt_string*)_arr.data.ptr;
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_scilexerasm_auto_completion_word_separators\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
    }
    _ret[_arr.len] = NULL;
    for (size_t i = 0; i < _arr.len; ++i) {
        libqt_string_free((libqt_string*)&_qstr[i]);
    }
    libqt_free(_arr.data.ptr);
    return _ret;
}

const char** q_scilexerasm_super_auto_completion_word_separators(void* self) {
    libqt_list _arr = QsciLexerAsm_SuperAutoCompletionWordSeparators((QsciLexerAsm*)self);
    const libqt_string* _qstr = (libqt_string*)_arr.data.ptr;
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_scilexerasm_auto_completion_word_separators\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
    }
    _ret[_arr.len] = NULL;
    for (size_t i = 0; i < _arr.len; ++i) {
        libqt_string_free((libqt_string*)&_qstr[i]);
    }
    libqt_free(_arr.data.ptr);
    return _ret;
}

void q_scilexerasm_on_auto_completion_word_separators(void* self, const char** (*callback)()) {
    QsciLexerAsm_OnAutoCompletionWordSeparators((QsciLexerAsm*)self, (intptr_t)callback);
}

const char* q_scilexerasm_block_end(void* self, int* style) {
    return QsciLexerAsm_BlockEnd((QsciLexerAsm*)self, style);
}

const char* q_scilexerasm_super_block_end(void* self, int* style) {
    return QsciLexerAsm_SuperBlockEnd((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_block_end(void* self, const char* (*callback)(void*, int*)) {
    QsciLexerAsm_OnBlockEnd((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_block_lookback(void* self) {
    return QsciLexerAsm_BlockLookback((QsciLexerAsm*)self);
}

int32_t q_scilexerasm_super_block_lookback(void* self) {
    return QsciLexerAsm_SuperBlockLookback((QsciLexerAsm*)self);
}

void q_scilexerasm_on_block_lookback(void* self, int32_t (*callback)()) {
    QsciLexerAsm_OnBlockLookback((QsciLexerAsm*)self, (intptr_t)callback);
}

const char* q_scilexerasm_block_start(void* self, int* style) {
    return QsciLexerAsm_BlockStart((QsciLexerAsm*)self, style);
}

const char* q_scilexerasm_super_block_start(void* self, int* style) {
    return QsciLexerAsm_SuperBlockStart((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_block_start(void* self, const char* (*callback)(void*, int*)) {
    QsciLexerAsm_OnBlockStart((QsciLexerAsm*)self, (intptr_t)callback);
}

const char* q_scilexerasm_block_start_keyword(void* self, int* style) {
    return QsciLexerAsm_BlockStartKeyword((QsciLexerAsm*)self, style);
}

const char* q_scilexerasm_super_block_start_keyword(void* self, int* style) {
    return QsciLexerAsm_SuperBlockStartKeyword((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_block_start_keyword(void* self, const char* (*callback)(void*, int*)) {
    QsciLexerAsm_OnBlockStartKeyword((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_brace_style(void* self) {
    return QsciLexerAsm_BraceStyle((QsciLexerAsm*)self);
}

int32_t q_scilexerasm_super_brace_style(void* self) {
    return QsciLexerAsm_SuperBraceStyle((QsciLexerAsm*)self);
}

void q_scilexerasm_on_brace_style(void* self, int32_t (*callback)()) {
    QsciLexerAsm_OnBraceStyle((QsciLexerAsm*)self, (intptr_t)callback);
}

bool q_scilexerasm_case_sensitive(void* self) {
    return QsciLexerAsm_CaseSensitive((QsciLexerAsm*)self);
}

bool q_scilexerasm_super_case_sensitive(void* self) {
    return QsciLexerAsm_SuperCaseSensitive((QsciLexerAsm*)self);
}

void q_scilexerasm_on_case_sensitive(void* self, bool (*callback)()) {
    QsciLexerAsm_OnCaseSensitive((QsciLexerAsm*)self, (intptr_t)callback);
}

QColor* q_scilexerasm_color(void* self, int style) {
    return QsciLexerAsm_Color((QsciLexerAsm*)self, style);
}

QColor* q_scilexerasm_super_color(void* self, int style) {
    return QsciLexerAsm_SuperColor((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_color(void* self, QColor* (*callback)(void*, int)) {
    QsciLexerAsm_OnColor((QsciLexerAsm*)self, (intptr_t)callback);
}

bool q_scilexerasm_eol_fill(void* self, int style) {
    return QsciLexerAsm_EolFill((QsciLexerAsm*)self, style);
}

bool q_scilexerasm_super_eol_fill(void* self, int style) {
    return QsciLexerAsm_SuperEolFill((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_eol_fill(void* self, bool (*callback)(void*, int)) {
    QsciLexerAsm_OnEolFill((QsciLexerAsm*)self, (intptr_t)callback);
}

QFont* q_scilexerasm_font(void* self, int style) {
    return QsciLexerAsm_Font((QsciLexerAsm*)self, style);
}

QFont* q_scilexerasm_super_font(void* self, int style) {
    return QsciLexerAsm_SuperFont((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_font(void* self, QFont* (*callback)(void*, int)) {
    QsciLexerAsm_OnFont((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_indentation_guide_view(void* self) {
    return QsciLexerAsm_IndentationGuideView((QsciLexerAsm*)self);
}

int32_t q_scilexerasm_super_indentation_guide_view(void* self) {
    return QsciLexerAsm_SuperIndentationGuideView((QsciLexerAsm*)self);
}

void q_scilexerasm_on_indentation_guide_view(void* self, int32_t (*callback)()) {
    QsciLexerAsm_OnIndentationGuideView((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_default_style(void* self) {
    return QsciLexerAsm_DefaultStyle((QsciLexerAsm*)self);
}

int32_t q_scilexerasm_super_default_style(void* self) {
    return QsciLexerAsm_SuperDefaultStyle((QsciLexerAsm*)self);
}

void q_scilexerasm_on_default_style(void* self, int32_t (*callback)()) {
    QsciLexerAsm_OnDefaultStyle((QsciLexerAsm*)self, (intptr_t)callback);
}

QColor* q_scilexerasm_paper(void* self, int style) {
    return QsciLexerAsm_Paper((QsciLexerAsm*)self, style);
}

QColor* q_scilexerasm_super_paper(void* self, int style) {
    return QsciLexerAsm_SuperPaper((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_paper(void* self, QColor* (*callback)(void*, int)) {
    QsciLexerAsm_OnPaper((QsciLexerAsm*)self, (intptr_t)callback);
}

QColor* q_scilexerasm_default_color2(void* self, int style) {
    return QsciLexerAsm_DefaultColor2((QsciLexerAsm*)self, style);
}

QColor* q_scilexerasm_super_default_color2(void* self, int style) {
    return QsciLexerAsm_SuperDefaultColor2((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_default_color2(void* self, QColor* (*callback)(void*, int)) {
    QsciLexerAsm_OnDefaultColor2((QsciLexerAsm*)self, (intptr_t)callback);
}

QFont* q_scilexerasm_default_font2(void* self, int style) {
    return QsciLexerAsm_DefaultFont2((QsciLexerAsm*)self, style);
}

QFont* q_scilexerasm_super_default_font2(void* self, int style) {
    return QsciLexerAsm_SuperDefaultFont2((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_default_font2(void* self, QFont* (*callback)(void*, int)) {
    QsciLexerAsm_OnDefaultFont2((QsciLexerAsm*)self, (intptr_t)callback);
}

QColor* q_scilexerasm_default_paper2(void* self, int style) {
    return QsciLexerAsm_DefaultPaper2((QsciLexerAsm*)self, style);
}

QColor* q_scilexerasm_super_default_paper2(void* self, int style) {
    return QsciLexerAsm_SuperDefaultPaper2((QsciLexerAsm*)self, style);
}

void q_scilexerasm_on_default_paper2(void* self, QColor* (*callback)(void*, int)) {
    QsciLexerAsm_OnDefaultPaper2((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_set_editor(void* self, void* editor) {
    QsciLexerAsm_SetEditor((QsciLexerAsm*)self, (QsciScintilla*)editor);
}

void q_scilexerasm_super_set_editor(void* self, void* editor) {
    QsciLexerAsm_SuperSetEditor((QsciLexerAsm*)self, (QsciScintilla*)editor);
}

void q_scilexerasm_on_set_editor(void* self, void (*callback)(void*, void*)) {
    QsciLexerAsm_OnSetEditor((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_style_bits_needed(void* self) {
    return QsciLexerAsm_StyleBitsNeeded((QsciLexerAsm*)self);
}

int32_t q_scilexerasm_super_style_bits_needed(void* self) {
    return QsciLexerAsm_SuperStyleBitsNeeded((QsciLexerAsm*)self);
}

void q_scilexerasm_on_style_bits_needed(void* self, int32_t (*callback)()) {
    QsciLexerAsm_OnStyleBitsNeeded((QsciLexerAsm*)self, (intptr_t)callback);
}

const char* q_scilexerasm_word_characters(void* self) {
    return QsciLexerAsm_WordCharacters((QsciLexerAsm*)self);
}

const char* q_scilexerasm_super_word_characters(void* self) {
    return QsciLexerAsm_SuperWordCharacters((QsciLexerAsm*)self);
}

void q_scilexerasm_on_word_characters(void* self, const char* (*callback)()) {
    QsciLexerAsm_OnWordCharacters((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_set_auto_indent_style(void* self, int autoindentstyle) {
    QsciLexerAsm_SetAutoIndentStyle((QsciLexerAsm*)self, autoindentstyle);
}

void q_scilexerasm_super_set_auto_indent_style(void* self, int autoindentstyle) {
    QsciLexerAsm_SuperSetAutoIndentStyle((QsciLexerAsm*)self, autoindentstyle);
}

void q_scilexerasm_on_set_auto_indent_style(void* self, void (*callback)(void*, int)) {
    QsciLexerAsm_OnSetAutoIndentStyle((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_set_color(void* self, void* c, int style) {
    QsciLexerAsm_SetColor((QsciLexerAsm*)self, (QColor*)c, style);
}

void q_scilexerasm_super_set_color(void* self, void* c, int style) {
    QsciLexerAsm_SuperSetColor((QsciLexerAsm*)self, (QColor*)c, style);
}

void q_scilexerasm_on_set_color(void* self, void (*callback)(void*, void*, int)) {
    QsciLexerAsm_OnSetColor((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_set_eol_fill(void* self, bool eoffill, int style) {
    QsciLexerAsm_SetEolFill((QsciLexerAsm*)self, eoffill, style);
}

void q_scilexerasm_super_set_eol_fill(void* self, bool eoffill, int style) {
    QsciLexerAsm_SuperSetEolFill((QsciLexerAsm*)self, eoffill, style);
}

void q_scilexerasm_on_set_eol_fill(void* self, void (*callback)(void*, bool, int)) {
    QsciLexerAsm_OnSetEolFill((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_set_font(void* self, void* f, int style) {
    QsciLexerAsm_SetFont((QsciLexerAsm*)self, (QFont*)f, style);
}

void q_scilexerasm_super_set_font(void* self, void* f, int style) {
    QsciLexerAsm_SuperSetFont((QsciLexerAsm*)self, (QFont*)f, style);
}

void q_scilexerasm_on_set_font(void* self, void (*callback)(void*, void*, int)) {
    QsciLexerAsm_OnSetFont((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_set_paper(void* self, void* c, int style) {
    QsciLexerAsm_SetPaper((QsciLexerAsm*)self, (QColor*)c, style);
}

void q_scilexerasm_super_set_paper(void* self, void* c, int style) {
    QsciLexerAsm_SuperSetPaper((QsciLexerAsm*)self, (QColor*)c, style);
}

void q_scilexerasm_on_set_paper(void* self, void (*callback)(void*, void*, int)) {
    QsciLexerAsm_OnSetPaper((QsciLexerAsm*)self, (intptr_t)callback);
}

bool q_scilexerasm_event(void* self, void* event) {
    return QsciLexerAsm_Event((QsciLexerAsm*)self, (QEvent*)event);
}

bool q_scilexerasm_super_event(void* self, void* event) {
    return QsciLexerAsm_SuperEvent((QsciLexerAsm*)self, (QEvent*)event);
}

void q_scilexerasm_on_event(void* self, bool (*callback)(void*, void*)) {
    QsciLexerAsm_OnEvent((QsciLexerAsm*)self, (intptr_t)callback);
}

bool q_scilexerasm_event_filter(void* self, void* watched, void* event) {
    return QsciLexerAsm_EventFilter((QsciLexerAsm*)self, (QObject*)watched, (QEvent*)event);
}

bool q_scilexerasm_super_event_filter(void* self, void* watched, void* event) {
    return QsciLexerAsm_SuperEventFilter((QsciLexerAsm*)self, (QObject*)watched, (QEvent*)event);
}

void q_scilexerasm_on_event_filter(void* self, bool (*callback)(void*, void*, void*)) {
    QsciLexerAsm_OnEventFilter((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_timer_event(void* self, void* event) {
    QsciLexerAsm_TimerEvent((QsciLexerAsm*)self, (QTimerEvent*)event);
}

void q_scilexerasm_super_timer_event(void* self, void* event) {
    QsciLexerAsm_SuperTimerEvent((QsciLexerAsm*)self, (QTimerEvent*)event);
}

void q_scilexerasm_on_timer_event(void* self, void (*callback)(void*, void*)) {
    QsciLexerAsm_OnTimerEvent((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_child_event(void* self, void* event) {
    QsciLexerAsm_ChildEvent((QsciLexerAsm*)self, (QChildEvent*)event);
}

void q_scilexerasm_super_child_event(void* self, void* event) {
    QsciLexerAsm_SuperChildEvent((QsciLexerAsm*)self, (QChildEvent*)event);
}

void q_scilexerasm_on_child_event(void* self, void (*callback)(void*, void*)) {
    QsciLexerAsm_OnChildEvent((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_custom_event(void* self, void* event) {
    QsciLexerAsm_CustomEvent((QsciLexerAsm*)self, (QEvent*)event);
}

void q_scilexerasm_super_custom_event(void* self, void* event) {
    QsciLexerAsm_SuperCustomEvent((QsciLexerAsm*)self, (QEvent*)event);
}

void q_scilexerasm_on_custom_event(void* self, void (*callback)(void*, void*)) {
    QsciLexerAsm_OnCustomEvent((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_connect_notify(void* self, void* signal) {
    QsciLexerAsm_ConnectNotify((QsciLexerAsm*)self, (QMetaMethod*)signal);
}

void q_scilexerasm_super_connect_notify(void* self, void* signal) {
    QsciLexerAsm_SuperConnectNotify((QsciLexerAsm*)self, (QMetaMethod*)signal);
}

void q_scilexerasm_on_connect_notify(void* self, void (*callback)(void*, void*)) {
    QsciLexerAsm_OnConnectNotify((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_disconnect_notify(void* self, void* signal) {
    QsciLexerAsm_DisconnectNotify((QsciLexerAsm*)self, (QMetaMethod*)signal);
}

void q_scilexerasm_super_disconnect_notify(void* self, void* signal) {
    QsciLexerAsm_SuperDisconnectNotify((QsciLexerAsm*)self, (QMetaMethod*)signal);
}

void q_scilexerasm_on_disconnect_notify(void* self, void (*callback)(void*, void*)) {
    QsciLexerAsm_OnDisconnectNotify((QsciLexerAsm*)self, (intptr_t)callback);
}

char* q_scilexerasm_text_as_bytes(void* self, const char* text) {
    libqt_string _str = QsciLexerAsm_TextAsBytes((QsciLexerAsm*)self, qstring(text));
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

char* q_scilexerasm_super_text_as_bytes(void* self, const char* text) {
    libqt_string _str = QsciLexerAsm_SuperTextAsBytes((QsciLexerAsm*)self, qstring(text));
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void q_scilexerasm_on_text_as_bytes(void* self, libqt_string (*callback)(void*, const char*)) {
    QsciLexerAsm_OnTextAsBytes((QsciLexerAsm*)self, (intptr_t)callback);
}

const char* q_scilexerasm_bytes_as_text(void* self, const char* bytes, int size) {
    libqt_string _str = QsciLexerAsm_BytesAsText((QsciLexerAsm*)self, bytes, size);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

const char* q_scilexerasm_super_bytes_as_text(void* self, const char* bytes, int size) {
    libqt_string _str = QsciLexerAsm_SuperBytesAsText((QsciLexerAsm*)self, bytes, size);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void q_scilexerasm_on_bytes_as_text(void* self, const char* (*callback)(void*, const char*, int)) {
    QsciLexerAsm_OnBytesAsText((QsciLexerAsm*)self, (intptr_t)callback);
}

QObject* q_scilexerasm_sender(void* self) {
    return QsciLexerAsm_Sender((QsciLexerAsm*)self);
}

QObject* q_scilexerasm_super_sender(void* self) {
    return QsciLexerAsm_SuperSender((QsciLexerAsm*)self);
}

void q_scilexerasm_on_sender(void* self, QObject* (*callback)()) {
    QsciLexerAsm_OnSender((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_sender_signal_index(void* self) {
    return QsciLexerAsm_SenderSignalIndex((QsciLexerAsm*)self);
}

int32_t q_scilexerasm_super_sender_signal_index(void* self) {
    return QsciLexerAsm_SuperSenderSignalIndex((QsciLexerAsm*)self);
}

void q_scilexerasm_on_sender_signal_index(void* self, int32_t (*callback)()) {
    QsciLexerAsm_OnSenderSignalIndex((QsciLexerAsm*)self, (intptr_t)callback);
}

int32_t q_scilexerasm_receivers(void* self, const char* signal) {
    return QsciLexerAsm_Receivers((QsciLexerAsm*)self, signal);
}

int32_t q_scilexerasm_super_receivers(void* self, const char* signal) {
    return QsciLexerAsm_SuperReceivers((QsciLexerAsm*)self, signal);
}

void q_scilexerasm_on_receivers(void* self, int32_t (*callback)(void*, const char*)) {
    QsciLexerAsm_OnReceivers((QsciLexerAsm*)self, (intptr_t)callback);
}

bool q_scilexerasm_is_signal_connected(void* self, void* signal) {
    return QsciLexerAsm_IsSignalConnected((QsciLexerAsm*)self, (QMetaMethod*)signal);
}

bool q_scilexerasm_super_is_signal_connected(void* self, void* signal) {
    return QsciLexerAsm_SuperIsSignalConnected((QsciLexerAsm*)self, (QMetaMethod*)signal);
}

void q_scilexerasm_on_is_signal_connected(void* self, bool (*callback)(void*, void*)) {
    QsciLexerAsm_OnIsSignalConnected((QsciLexerAsm*)self, (intptr_t)callback);
}

void q_scilexerasm_on_object_name_changed(void* self, void (*callback)(void*, const char*)) {
    QObject_Connect_ObjectNameChanged((QObject*)self, (intptr_t)callback);
}

void q_scilexerasm_delete(void* self) {
    QsciLexerAsm_Delete((QsciLexerAsm*)(self));
}
