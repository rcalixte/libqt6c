...
void q_undoview_on_indexes_moved(void* self, void (*callback)(void*, libqt_list /* of QModelIndex* */)) {
...
const char** q_undoview_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_undoview_dynamic_property_names\n");
        abort();
    }
