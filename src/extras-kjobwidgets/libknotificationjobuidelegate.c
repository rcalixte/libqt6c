...
void k_notificationjobuidelegate_set_object_name(void* self, const char* name) {
...
int32_t k_notificationjobuidelegate_start_timer2(void* self, int64_t time) {
...
const char** k_notificationjobuidelegate_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_notificationjobuidelegate_dynamic_property_names\n");
        abort();
    }
...
int32_t k_notificationjobuidelegate_start_timer23(void* self, int64_t time, int32_t timerType) {
