...
KToolTipHelper* k_tooltiphelper_instance() {
    return KToolTipHelper_Instance();
}

bool k_tooltiphelper_event_filter(void* self, void* watched, void* event) {
    return KToolTipHelper_EventFilter((KToolTipHelper*)self, (QObject*)watched, (QEvent*)event);
}

const char* k_tooltiphelper_whats_this_hint_only() {
    libqt_string _str = KToolTipHelper_WhatsThisHintOnly();
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_tooltiphelper_tr2(const char* s, const char* c) {
    libqt_string _str = KToolTipHelper_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_tooltiphelper_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KToolTipHelper_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** k_tooltiphelper_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_tooltiphelper_dynamic_property_names");
        abort();
    }
