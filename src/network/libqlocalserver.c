...
const char** q_localserver_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_localserver_dynamic_property_names");
        abort();
    }
...
QMetaObject__Connection* q_localserver_connect5(void* sender, void* signal, void* receiver, void* method, int32_t type) {
