...
KIconLoader* k_iconloader_new3(const char* appname, const char* extraSearchPaths[]) {
    ...
    if (extraSearchPaths_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_new3");
        abort();
    }
...
KIconLoader* k_iconloader_new4(const char* appname, const char* extraSearchPaths[], void* parent) {
    ...
    libqt_string* extraSearchPaths_qstr = (libqt_string*)malloc(extraSearchPaths_len * sizeof(libqt_string));
    if (extraSearchPaths_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_new4");
        abort();
    }
    for (size_t i = 0; i < extraSearchPaths_len; ++i) {
        extraSearchPaths_qstr[i] = qstring(extraSearchPaths[i]);
...
const char* k_iconloader_tr(const char* s) {
    libqt_string _str = KIconLoader_Tr(s);
...
const char** k_iconloader_load_animated(void* self, const char* name, int32_t group) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_load_animated");
        abort();
    }
...
const char** k_iconloader_query_icons(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_query_icons");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_iconloader_query_icons2(void* self, int group_or_size) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_query_icons2");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_iconloader_query_icons_by_context(void* self, int group_or_size) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_query_icons_by_context");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_iconloader_query_icons_by_dir(void* self, const char* iconsDir) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_query_icons_by_dir");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_iconloader_search_paths(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_search_paths");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
void k_iconloader_draw_overlays(void* self, const char* overlays[], void* pixmap, int32_t group) {
    ...
    if (overlays_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_draw_overlays");
        abort();
    }
...
const char* k_iconloader_tr2(const char* s, const char* c) {
    libqt_string _str = KIconLoader_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_iconloader_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KIconLoader_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
QPixmap* k_iconloader_load_icon5(void* self, const char* name, int32_t group, int size, int state, const char* overlays[]) {
    ...
    libqt_string* overlays_qstr = (libqt_string*)malloc(overlays_len * sizeof(libqt_string));
    if (overlays_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_load_icon5");
        abort();
    }
    for (size_t i = 0; i < overlays_len; ++i) {
        overlays_qstr[i] = qstring(overlays[i]);
...
QPixmap* k_iconloader_load_mime_type_icon5(void* self, const char* iconName, int32_t group, int size, int state, const char* overlays[]) {
    ...
    libqt_string* overlays_qstr = (libqt_string*)malloc(overlays_len * sizeof(libqt_string));
    if (overlays_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_load_mime_type_icon5");
        abort();
    }
    for (size_t i = 0; i < overlays_len; ++i) {
        overlays_qstr[i] = qstring(overlays[i]);
...
const char** k_iconloader_load_animated3(void* self, const char* name, int32_t group, int size) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_load_animated3");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_iconloader_query_icons22(void* self, int group_or_size, int32_t context) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_query_icons22");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_iconloader_query_icons_by_context2(void* self, int group_or_size, int32_t context) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_query_icons_by_context2");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
void k_iconloader_reconfigure2(void* self, const char* appname, const char* extraSearchPaths[]) {
    ...
    libqt_string* extraSearchPaths_qstr = (libqt_string*)malloc(extraSearchPaths_len * sizeof(libqt_string));
    if (extraSearchPaths_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_reconfigure2");
        abort();
    }
    for (size_t i = 0; i < extraSearchPaths_len; ++i) {
        extraSearchPaths_qstr[i] = qstring(extraSearchPaths[i]);
...
void k_iconloader_draw_overlays4(void* self, const char* overlays[], void* pixmap, int32_t group, int state) {
    ...
    libqt_string* overlays_qstr = (libqt_string*)malloc(overlays_len * sizeof(libqt_string));
    if (overlays_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_draw_overlays4");
        abort();
    }
    for (size_t i = 0; i < overlays_len; ++i) {
        overlays_qstr[i] = qstring(overlays[i]);
...
const char** k_iconloader_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_iconloader_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
QIcon* k_de_icon3(const char* param1, const char* param2[], void* param3) {
    ...
    if (param2_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_de_icon3");
        abort();
    }
