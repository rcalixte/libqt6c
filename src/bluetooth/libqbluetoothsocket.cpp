...
void QBluetoothSocket_ErrorOccurred(QBluetoothSocket* self, int error) {
    self->errorOccurred(static_cast<QBluetoothSocket::SocketError>(error));
}
...
void QBluetoothSocket_Connect_ErrorOccurred(QBluetoothSocket* self, intptr_t slot) {
    ...
    QBluetoothSocket::connect(self, &QBluetoothSocket::errorOccurred, [self, slotFunc](QBluetoothSocket::SocketError error) {
...
// Derived class handler implementation
void QBluetoothSocket_SetSocketError(QBluetoothSocket* self, int error) {
    auto* vqbluetoothsocket = dynamic_cast<VirtualQBluetoothSocket*>(self);
    if (vqbluetoothsocket && vqbluetoothsocket->isVirtualQBluetoothSocket) {
        vqbluetoothsocket->setSocketError(static_cast<QBluetoothSocket::SocketError>(error));
    } else {
        ((VirtualQBluetoothSocket*)self)->setSocketError(static_cast<QBluetoothSocket::SocketError>(error));
    }
}

// Base class handler implementation
void QBluetoothSocket_SuperSetSocketError(QBluetoothSocket* self, int error) {
    auto* vqbluetoothsocket = dynamic_cast<VirtualQBluetoothSocket*>(self);
    if (vqbluetoothsocket && vqbluetoothsocket->isVirtualQBluetoothSocket) {
