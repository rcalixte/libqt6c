...
libqt_list /* of Poppler__Annotation* */ Poppler__Page_Annotations2(const Poppler__Page* self, const libqt_list /* set of int */ subtypes) {
    ...
    int* subtypes_setarr = static_cast<int*>(subtypes.data.ints);
...
QDateTime* Poppler__Document_Date(const Poppler__Document* self, const libqt_string type) {
    ...
    return new QDateTime(self->date(type_QString));
}
...
libqt_string Poppler__Document_Info(const Poppler__Document* self, const libqt_string type) {
    QString type_QString = QString::fromUtf8(type.data, type.len);
    QString _ret = self->info(type_QString);
    // Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
    QByteArray _b = _ret.toUtf8();
    libqt_string _str;
    _str.len = _b.length();
...
Poppler__Link* Poppler__Document_AdditionalAction(const Poppler__Document* self, int type) {
    return self->additionalAction(static_cast<Poppler::Document::DocumentAdditionalActionsType>(type)).release();
}
