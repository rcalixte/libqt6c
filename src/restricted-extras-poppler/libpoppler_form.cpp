...
Poppler__Link* Poppler__FormField_AdditionalAction(const Poppler__FormField* self, int type) {
    return self->additionalAction(static_cast<Poppler::FormField::AdditionalActionType>(type)).release();
}
...
void Poppler__FormFieldChoice_SetCurrentChoices(Poppler__FormFieldChoice* self, const libqt_list /* of int */ choice) {
    ...
    int* choice_arr = static_cast<int*>(choice.data.ints);
