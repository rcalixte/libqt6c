...
const char* k_localizedstring_to_string2(void* self, const char* languages[]) {
    ...
    if (languages_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_localizedstring_to_string2");
        abort();
    }
...
KLocalizedString* k_localizedstring_with_languages(void* self, const char* languages[]) {
    ...
    libqt_string* languages_qstr = (libqt_string*)malloc(languages_len * sizeof(libqt_string));
    if (languages_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_localizedstring_with_languages");
        abort();
    }
    for (size_t i = 0; i < languages_len; ++i) {
        languages_qstr[i] = qstring(languages[i]);
...
const char** k_localizedstring_languages() {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_localizedstring_languages");
        abort();
    }
...
void k_localizedstring_set_languages(const char* languages[]) {
    ...
    libqt_string* languages_qstr = (libqt_string*)malloc(languages_len * sizeof(libqt_string));
    if (languages_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_localizedstring_set_languages");
        abort();
    }
    for (size_t i = 0; i < languages_len; ++i) {
        languages_qstr[i] = qstring(languages[i]);
