...
const char* q_combobox_tr(const char* s) {
    libqt_string _str = QComboBox_Tr(s);
...
void q_combobox_add_items(void* self, const char* texts[]) {
    ...
    if (texts_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_combobox_add_items");
        abort();
    }
...
void q_combobox_insert_items(void* self, int index, const char* texts[]) {
    ...
    libqt_string* texts_qstr = (libqt_string*)malloc(texts_len * sizeof(libqt_string));
    if (texts_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in q_combobox_insert_items");
        abort();
    }
    for (size_t i = 0; i < texts_len; ++i) {
        texts_qstr[i] = qstring(texts[i]);
...
const char* q_combobox_tr2(const char* s, const char* c) {
    libqt_string _str = QComboBox_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* q_combobox_tr3(const char* s, const char* c, int n) {
    libqt_string _str = QComboBox_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** q_combobox_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_combobox_dynamic_property_names");
        abort();
    }
...
bool q_combobox_move_to_thread2(void* self, void* thread, void* param2) {
...
QMetaObject__Connection* q_combobox_connect5(void* sender, void* signal, void* receiver, void* method, int32_t type) {
