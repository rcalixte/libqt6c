...
QCommandLineOption* q_commandlineoption_new2(const char* names[static 1]) {
    ...
    if (names_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_commandlineoption_new2");
        abort();
    }
...
QCommandLineOption* q_commandlineoption_new4(const char* names[static 1], const char* description) {
    ...
    libqt_string* names_qstr = (libqt_string*)malloc(names_len * sizeof(libqt_string));
    if (names_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_commandlineoption_new4");
        abort();
    }
    for (size_t i = 0; i < names_len; ++i) {
        names_qstr[i] = qstring(names[i]);
...
QCommandLineOption* q_commandlineoption_new8(const char* names[static 1], const char* description, const char* valueName) {
    ...
    libqt_string* names_qstr = (libqt_string*)malloc(names_len * sizeof(libqt_string));
    if (names_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_commandlineoption_new8");
        abort();
    }
    for (size_t i = 0; i < names_len; ++i) {
        names_qstr[i] = qstring(names[i]);
...
QCommandLineOption* q_commandlineoption_new9(const char* names[static 1], const char* description, const char* valueName, const char* defaultValue) {
    ...
    libqt_string* names_qstr = (libqt_string*)malloc(names_len * sizeof(libqt_string));
    if (names_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_commandlineoption_new9");
        abort();
    }
    for (size_t i = 0; i < names_len; ++i) {
        names_qstr[i] = qstring(names[i]);
...
const char** q_commandlineoption_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_commandlineoption_names");
        abort();
    }
...
void q_commandlineoption_set_default_values(void* self, const char* defaultValues[static 1]) {
    ...
    if (defaultValues_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_commandlineoption_set_default_values");
        abort();
    }
...
const char** q_commandlineoption_default_values(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_commandlineoption_default_values");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
