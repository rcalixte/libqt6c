...
void k_codecaction_set_items(void* self, const char* lst[static 1]) {
    ...
    if (lst_qstr == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_codecaction_set_items\n");
        abort();
    }
...
const char** k_codecaction_items(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_codecaction_items\n");
        abort();
    }
...
int32_t k_codecaction_start_timer2(void* self, int64_t time) {
...
const char** k_codecaction_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_codecaction_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
int32_t k_codecaction_start_timer23(void* self, int64_t time, int32_t timerType) {
