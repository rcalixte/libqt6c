...
const char* k_fontaction_tr(const char* s) {
    libqt_string _str = KFontAction_Tr(s);
...
const char* k_fontaction_tr2(const char* s, const char* c) {
    libqt_string _str = KFontAction_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_fontaction_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KFontAction_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
void k_fontaction_set_items(void* self, const char* lst[static 1]) {
    ...
    if (lst_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_fontaction_set_items");
        abort();
    }
...
const char** k_fontaction_items(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_fontaction_items");
        abort();
    }
...
const char** k_fontaction_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_fontaction_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
