...
const char* k_viewstateserializer_tr(const char* s) {
    libqt_string _str = KViewStateSerializer_Tr(s);
...
const char** k_viewstateserializer_selection_keys(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_viewstateserializer_selection_keys");
        abort();
    }
...
const char** k_viewstateserializer_expansion_keys(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_viewstateserializer_expansion_keys");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
void k_viewstateserializer_restore_selection(void* self, const char* indexStrings[]) {
    ...
    if (indexStrings_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_viewstateserializer_restore_selection");
        abort();
    }
...
void k_viewstateserializer_restore_expanded(void* self, const char* indexStrings[]) {
    ...
    libqt_string* indexStrings_qstr = (libqt_string*)malloc(indexStrings_len * sizeof(libqt_string));
    if (indexStrings_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_viewstateserializer_restore_expanded");
        abort();
    }
    for (size_t i = 0; i < indexStrings_len; ++i) {
        indexStrings_qstr[i] = qstring(indexStrings[i]);
...
const char* k_viewstateserializer_tr2(const char* s, const char* c) {
    libqt_string _str = KViewStateSerializer_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_viewstateserializer_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KViewStateSerializer_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** k_viewstateserializer_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_viewstateserializer_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
QMetaObject__Connection* k_viewstateserializer_connect5(void* sender, void* signal, void* receiver, void* method, int32_t typeVal) {
    return QObject_Connect5((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method, typeVal);
}

QMetaObject__Connection* k_viewstateserializer_connect4(void* self, void* sender, const char* signal, const char* member, int32_t typeVal) {
