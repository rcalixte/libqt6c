...
void k_fontsizeaction_set_items(void* self, const char* lst[]) {
    ...
    if (lst_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_fontsizeaction_set_items");
        abort();
    }
...
const char** k_fontsizeaction_items(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_fontsizeaction_items");
        abort();
    }
...
const char** k_fontsizeaction_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_fontsizeaction_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
QMetaObject__Connection* k_fontsizeaction_connect5(void* sender, void* signal, void* receiver, void* method, int32_t typeVal) {
    return QObject_Connect5((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method, typeVal);
}

QMetaObject__Connection* k_fontsizeaction_connect4(void* self, void* sender, const char* signal, const char* member, int32_t typeVal) {
