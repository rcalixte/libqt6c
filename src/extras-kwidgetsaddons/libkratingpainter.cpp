...
void KRatingPainter_SetAlignment(KRatingPainter* self, int align) {
    self->setAlignment(static_cast<Qt::Alignment>(align));
}
...
void KRatingPainter_PaintRating(QPainter* p, const QRect* rect, int align, int rating) {
    KRatingPainter::paintRating(p, *rect, static_cast<Qt::Alignment>(align), static_cast<int>(rating));
}

int KRatingPainter_GetRatingFromPosition(const QRect* rect, int align, int direction, const QPoint* pos) {
    return KRatingPainter::getRatingFromPosition(*rect, static_cast<Qt::Alignment>(align), static_cast<Qt::LayoutDirection>(direction), *pos);
}
...
void KRatingPainter_PaintRating5(QPainter* p, const QRect* rect, int align, int rating, int hoverRating) {
    KRatingPainter::paintRating(p, *rect, static_cast<Qt::Alignment>(align), static_cast<int>(rating), static_cast<int>(hoverRating));
}
