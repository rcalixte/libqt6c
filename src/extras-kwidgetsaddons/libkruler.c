...
const char* k_ruler_tr2(const char* s, const char* c) {
    libqt_string _str = KRuler_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_ruler_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KRuler_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
void k_ruler_grab_gesture2(void* self, int64_t typeVal, int32_t flags) {
...
const char** k_ruler_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_ruler_dynamic_property_names");
        abort();
    }
