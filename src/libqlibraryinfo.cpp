...
libqt_list /* of libqt_string */ QLibraryInfo_Paths(int p) {
    QList<QString> _ret = QLibraryInfo::paths(static_cast<QLibraryInfo::LibraryPath>(p));
...
libqt_list /* of libqt_string */ QLibraryInfo_PlatformPluginArguments(const libqt_string platformName) {
    ...
    QList<QString> _ret = QLibraryInfo::platformPluginArguments(platformName_QString);
    // Convert QList<> from C++ memory to manually-managed C memory
    libqt_string* _arr = static_cast<libqt_string*>(malloc(sizeof(libqt_string) * (_ret.size() + 1)));
    for (qsizetype i = 0; i < _ret.size(); ++i) {
        QString _lv_ret = _ret[i];
