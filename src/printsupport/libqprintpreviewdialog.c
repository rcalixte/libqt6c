...
void q_printpreviewdialog_grab_gesture2(void* self, int64_t typeVal, int32_t flags) {
...
const char** q_printpreviewdialog_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_printpreviewdialog_dynamic_property_names");
        abort();
    }
...
double q_printpreviewdialog_qbase_get_decoded_metric_f(void* self, int32_t metricA, int32_t metricB) {
    return QPrintPreviewDialog_QBaseGetDecodedMetricF((QPrintPreviewDialog*)self, metricA, metricB);
}
