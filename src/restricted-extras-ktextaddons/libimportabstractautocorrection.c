...
bool k_textautocorrectioncore__importabstractautocorrection_import(void* self, const char* fileName, const char* errorMessage, int32_t loadAttribute) {
    return TextAutoCorrectionCore__ImportAbstractAutocorrection_Import((TextAutoCorrectionCore__ImportAbstractAutocorrection*)self, qstring(fileName), qstring(errorMessage), loadAttribute);
}
...
libqt_map /* of const char* to const char* */ k_textautocorrectioncore__importabstractautocorrection_autocorrect_entries(void* self) {
    ...
    if (_ret_keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map string keys in k_textautocorrectioncore__importabstractautocorrection_autocorrect_entries\n");
        abort();
    }
    libqt_string* _out_values = (libqt_string*)_out.values;
    char** _ret_values = (char**)malloc(_ret.len * sizeof(char*));
    if (_ret_values == NULL) {
        fprintf(stderr, "Failed to allocate memory for map string values in k_textautocorrectioncore__importabstractautocorrection_autocorrect_entries\n");
        free(_out.keys);
        abort();
    }
    for (size_t i = 0; i < _ret.len; ++i) {
        ...
        if (_ret_keys[i] == NULL) {
            ...
            free(_ret_keys);
            fprintf(stderr, "Failed to allocate memory for map keys in k_textautocorrectioncore__importabstractautocorrection_autocorrect_entries\n");
            abort();
        }
...
libqt_map /* of const char* to const char* */ k_textautocorrectioncore__importabstractautocorrection_super_script_entries(void* self) {
    ...
    char** _ret_keys = (char**)malloc(_ret.len * sizeof(char*));
    if (_ret_keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map string keys in k_textautocorrectioncore__importabstractautocorrection_super_script_entries\n");
        abort();
    }
    libqt_string* _out_values = (libqt_string*)_out.values;
    char** _ret_values = (char**)malloc(_ret.len * sizeof(char*));
    if (_ret_values == NULL) {
        fprintf(stderr, "Failed to allocate memory for map string values in k_textautocorrectioncore__importabstractautocorrection_super_script_entries\n");
        free(_out.keys);
        abort();
    }
    for (size_t i = 0; i < _ret.len; ++i) {
        ...
        if (_ret_keys[i] == NULL) {
            ...
            free(_ret_keys);
            fprintf(stderr, "Failed to allocate memory for map keys in k_textautocorrectioncore__importabstractautocorrection_super_script_entries\n");
            abort();
        }
        memcpy(_ret_keys[i], _out_keys[i].data, _out_keys[i].len);
        _ret_keys[i][_out_keys[i].len] = '\0';
