...
int32_t q_coloraxis_start_timer2(void* self, int64_t time) {
...
const char** q_coloraxis_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_coloraxis_dynamic_property_names\n");
        abort();
    }
...
int32_t q_coloraxis_start_timer23(void* self, int64_t time, int32_t timerType) {
