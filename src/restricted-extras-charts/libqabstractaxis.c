...
const char** q_abstractaxis_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_abstractaxis_dynamic_property_names\n");
        abort();
    }
...
QMetaObject__Connection* q_abstractaxis_connect5(void* sender, void* signal, void* receiver, void* method, int32_t type) {
