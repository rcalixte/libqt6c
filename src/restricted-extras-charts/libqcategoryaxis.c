...
const char** q_categoryaxis_categories_labels(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_categoryaxis_categories_labels\n");
        abort();
    }
...
int32_t q_categoryaxis_start_timer2(void* self, int64_t time) {
...
const char** q_categoryaxis_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_categoryaxis_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
int32_t q_categoryaxis_start_timer23(void* self, int64_t time, int32_t timerType) {
