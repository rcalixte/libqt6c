...
libqt_list /* of KUserGroup* */ KUser_Groups(const KUser* self) {
    ...
    KUserGroup** _arr = static_cast<KUserGroup**>(malloc(sizeof(KUserGroup*) * (_ret.size())));
...
libqt_list /* of KUser* */ KUser_AllUsers() {
    ...
    // Convert QList<> from C++ memory to manually-managed C memory
    KUser** _arr = static_cast<KUser**>(malloc(sizeof(KUser*) * (_ret.size())));
    for (qsizetype i = 0; i < _ret.size(); ++i) {
