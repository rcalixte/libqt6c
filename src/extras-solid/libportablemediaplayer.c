...
const char** k_solid__portablemediaplayer_supported_protocols(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_solid__portablemediaplayer_supported_protocols\n");
        abort();
    }
...
const char** k_solid__portablemediaplayer_supported_drivers(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_solid__portablemediaplayer_supported_drivers\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_solid__portablemediaplayer_supported_drivers1(void* self, const char* protocol) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_solid__portablemediaplayer_supported_drivers1\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
int32_t k_solid__portablemediaplayer_start_timer2(void* self, int64_t time) {
...
const char** k_solid__portablemediaplayer_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_solid__portablemediaplayer_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
int32_t k_solid__portablemediaplayer_start_timer23(void* self, int64_t time, int32_t timerType) {
