#include "../extras-kcoreaddons/libkcompositejob.hpp"
#include "libdropjob.hpp"
#include "libjob_base.hpp"
#include "../extras-kcoreaddons/libkjob.hpp"
#include "../libqdatetime.hpp"
#include "../libqmetaobject.hpp"
#include "../libqobjectdefs.hpp"
#include "../libqobject.hpp"
#include "../libqurl.hpp"
#include "libcopyjob.hpp"
#include "libcopyjob.h"

const QMetaObject* k_io__copyjob_meta_object(void* self) {
    return KIO__CopyJob_MetaObject((KIO__CopyJob*)self);
}

void* k_io__copyjob_metacast(void* self, const char* param1) {
    return KIO__CopyJob_Metacast((KIO__CopyJob*)self, param1);
}

int32_t k_io__copyjob_metacall(void* self, int32_t param1, int param2, void* param3) {
    return KIO__CopyJob_Metacall((KIO__CopyJob*)self, param1, param2, param3);
}

const char* k_io__copyjob_tr(const char* s) {
    libqt_string _str = QObject_Tr(s);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

int32_t k_io__copyjob_operation_mode(void* self) {
    return KIO__CopyJob_OperationMode((KIO__CopyJob*)self);
}

libqt_list /* of QUrl* */ k_io__copyjob_src_urls(void* self) {
    libqt_list _arr = KIO__CopyJob_SrcUrls((KIO__CopyJob*)self);
    return _arr;
}

QUrl* k_io__copyjob_dest_url(void* self) {
    return KIO__CopyJob_DestUrl((KIO__CopyJob*)self);
}

void k_io__copyjob_set_default_permissions(void* self, bool b) {
    KIO__CopyJob_SetDefaultPermissions((KIO__CopyJob*)self, b);
}

void k_io__copyjob_set_auto_skip(void* self, bool autoSkip) {
    KIO__CopyJob_SetAutoSkip((KIO__CopyJob*)self, autoSkip);
}

void k_io__copyjob_set_auto_rename(void* self, bool autoRename) {
    KIO__CopyJob_SetAutoRename((KIO__CopyJob*)self, autoRename);
}

void k_io__copyjob_set_write_into_existing_directories(void* self, bool overwriteAllDirs) {
    KIO__CopyJob_SetWriteIntoExistingDirectories((KIO__CopyJob*)self, overwriteAllDirs);
}

bool k_io__copyjob_do_suspend(void* self) {
    return KIO__CopyJob_DoSuspend((KIO__CopyJob*)self);
}

bool k_io__copyjob_do_resume(void* self) {
    return KIO__CopyJob_DoResume((KIO__CopyJob*)self);
}

void k_io__copyjob_processed_files(void* self, void* job, uintptr_t files) {
    KIO__CopyJob_ProcessedFiles((KIO__CopyJob*)self, (KIO__Job*)job, files);
}

void k_io__copyjob_on_processed_files(void* self, void (*callback)(void*, void*, uintptr_t)) {
    KIO__CopyJob_Connect_ProcessedFiles((KIO__CopyJob*)self, (intptr_t)callback);
}

void k_io__copyjob_processed_dirs(void* self, void* job, uintptr_t dirs) {
    KIO__CopyJob_ProcessedDirs((KIO__CopyJob*)self, (KIO__Job*)job, dirs);
}

void k_io__copyjob_on_processed_dirs(void* self, void (*callback)(void*, void*, uintptr_t)) {
    KIO__CopyJob_Connect_ProcessedDirs((KIO__CopyJob*)self, (intptr_t)callback);
}

void k_io__copyjob_copying(void* self, void* job, void* src, void* dest) {
    KIO__CopyJob_Copying((KIO__CopyJob*)self, (KIO__Job*)job, (QUrl*)src, (QUrl*)dest);
}

void k_io__copyjob_on_copying(void* self, void (*callback)(void*, void*, void*, void*)) {
    KIO__CopyJob_Connect_Copying((KIO__CopyJob*)self, (intptr_t)callback);
}

void k_io__copyjob_linking(void* self, void* job, const char* target, void* to) {
    KIO__CopyJob_Linking((KIO__CopyJob*)self, (KIO__Job*)job, qstring(target), (QUrl*)to);
}

void k_io__copyjob_on_linking(void* self, void (*callback)(void*, void*, const char*, void*)) {
    KIO__CopyJob_Connect_Linking((KIO__CopyJob*)self, (intptr_t)callback);
}

void k_io__copyjob_moving(void* self, void* job, void* from, void* to) {
    KIO__CopyJob_Moving((KIO__CopyJob*)self, (KIO__Job*)job, (QUrl*)from, (QUrl*)to);
}

void k_io__copyjob_on_moving(void* self, void (*callback)(void*, void*, void*, void*)) {
    KIO__CopyJob_Connect_Moving((KIO__CopyJob*)self, (intptr_t)callback);
}

void k_io__copyjob_creating_dir(void* self, void* job, void* dir) {
    KIO__CopyJob_CreatingDir((KIO__CopyJob*)self, (KIO__Job*)job, (QUrl*)dir);
}

void k_io__copyjob_on_creating_dir(void* self, void (*callback)(void*, void*, void*)) {
    KIO__CopyJob_Connect_CreatingDir((KIO__CopyJob*)self, (intptr_t)callback);
}

void k_io__copyjob_renamed(void* self, void* job, void* from, void* to) {
    KIO__CopyJob_Renamed((KIO__CopyJob*)self, (KIO__Job*)job, (QUrl*)from, (QUrl*)to);
}

void k_io__copyjob_on_renamed(void* self, void (*callback)(void*, void*, void*, void*)) {
    KIO__CopyJob_Connect_Renamed((KIO__CopyJob*)self, (intptr_t)callback);
}

void k_io__copyjob_copying_done(void* self, void* job, void* from, void* to, void* mtime, bool directory, bool renamed) {
    KIO__CopyJob_CopyingDone((KIO__CopyJob*)self, (KIO__Job*)job, (QUrl*)from, (QUrl*)to, (QDateTime*)mtime, directory, renamed);
}

void k_io__copyjob_on_copying_done(void* self, void (*callback)(void*, void*, void*, void*, void*, bool, bool)) {
    KIO__CopyJob_Connect_CopyingDone((KIO__CopyJob*)self, (intptr_t)callback);
}

void k_io__copyjob_copying_link_done(void* self, void* job, void* from, const char* target, void* to) {
    KIO__CopyJob_CopyingLinkDone((KIO__CopyJob*)self, (KIO__Job*)job, (QUrl*)from, qstring(target), (QUrl*)to);
}

void k_io__copyjob_on_copying_link_done(void* self, void (*callback)(void*, void*, void*, const char*, void*)) {
    KIO__CopyJob_Connect_CopyingLinkDone((KIO__CopyJob*)self, (intptr_t)callback);
}

const char* k_io__copyjob_tr2(const char* s, const char* c) {
    libqt_string _str = QObject_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

const char* k_io__copyjob_tr3(const char* s, const char* c, int n) {
    libqt_string _str = QObject_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void k_io__copyjob_start(void* self) {
    KIO__Job_Start((KIO__Job*)self);
}

KIO__JobUiDelegateExtension* k_io__copyjob_ui_delegate_extension(void* self) {
    return KIO__Job_UiDelegateExtension((KIO__Job*)self);
}

void k_io__copyjob_set_ui_delegate_extension(void* self, void* extension) {
    KIO__Job_SetUiDelegateExtension((KIO__Job*)self, (KIO__JobUiDelegateExtension*)extension);
}

const char* k_io__copyjob_error_string(void* self) {
    libqt_string _str = KIO__Job_ErrorString((KIO__Job*)self);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

const char** k_io__copyjob_detailed_error_strings(void* self) {
    libqt_list _arr = KIO__Job_DetailedErrorStrings((KIO__Job*)self);
    const libqt_string* _qstr = (libqt_string*)_arr.data.ptr;
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__copyjob_detailed_error_strings\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
    }
    _ret[_arr.len] = NULL;
    for (size_t i = 0; i < _arr.len; ++i) {
        libqt_string_free((libqt_string*)&_qstr[i]);
    }
    libqt_free(_arr.data.ptr);
    return _ret;
}

void k_io__copyjob_set_parent_job(void* self, void* parentJob) {
    KIO__Job_SetParentJob((KIO__Job*)self, (KIO__Job*)parentJob);
}

KIO__Job* k_io__copyjob_parent_job(void* self) {
    return KIO__Job_ParentJob((KIO__Job*)self);
}

void k_io__copyjob_set_meta_data(void* self, void* metaData) {
    KIO__Job_SetMetaData((KIO__Job*)self, (KIO__MetaData*)metaData);
}

void k_io__copyjob_add_meta_data(void* self, const char* key, const char* value) {
    KIO__Job_AddMetaData((KIO__Job*)self, qstring(key), qstring(value));
}

void k_io__copyjob_add_meta_data2(void* self, libqt_map /* of const char* to const char* */ values) {
    // Convert libqt_map to QMap<QString,QString>
    libqt_map values_ret;
    values_ret.len = values.len;
    values_ret.keys = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in k_io__copyjob_add_meta_data2\n");
        abort();
    }
    values_ret.values = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.values == NULL) {
        free(values_ret.keys);
        fprintf(stderr, "Failed to allocate memory for map values in k_io__copyjob_add_meta_data2\n");
        abort();
    }
    const char** values_karr = (const char**)values.keys;
    libqt_string* values_kdest = (libqt_string*)values_ret.keys;
    const char** values_varr = (const char**)values.values;
    libqt_string* values_vdest = (libqt_string*)values_ret.values;
    for (size_t i = 0; i < values_ret.len; ++i) {
        values_kdest[i] = qstring(values_karr[i]);
        values_vdest[i] = qstring(values_varr[i]);
    }
    KIO__Job_AddMetaData2((KIO__Job*)self, values_ret);
    free(values_ret.keys);
    free(values_ret.values);
}

void k_io__copyjob_merge_meta_data(void* self, libqt_map /* of const char* to const char* */ values) {
    // Convert libqt_map to QMap<QString,QString>
    libqt_map values_ret;
    values_ret.len = values.len;
    values_ret.keys = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in k_io__copyjob_merge_meta_data\n");
        abort();
    }
    values_ret.values = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.values == NULL) {
        free(values_ret.keys);
        fprintf(stderr, "Failed to allocate memory for map values in k_io__copyjob_merge_meta_data\n");
        abort();
    }
    const char** values_karr = (const char**)values.keys;
    libqt_string* values_kdest = (libqt_string*)values_ret.keys;
    const char** values_varr = (const char**)values.values;
    libqt_string* values_vdest = (libqt_string*)values_ret.values;
    for (size_t i = 0; i < values_ret.len; ++i) {
        values_kdest[i] = qstring(values_karr[i]);
        values_vdest[i] = qstring(values_varr[i]);
    }
    KIO__Job_MergeMetaData((KIO__Job*)self, values_ret);
    free(values_ret.keys);
    free(values_ret.values);
}

KIO__MetaData* k_io__copyjob_outgoing_meta_data(void* self) {
    return KIO__Job_OutgoingMetaData((KIO__Job*)self);
}

KIO__MetaData* k_io__copyjob_meta_data(void* self) {
    return KIO__Job_MetaData((KIO__Job*)self);
}

const char* k_io__copyjob_query_meta_data(void* self, const char* key) {
    libqt_string _str = KIO__Job_QueryMetaData((KIO__Job*)self, qstring(key));
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void k_io__copyjob_connected(void* self, void* job) {
    KIO__Job_Connected((KIO__Job*)self, (KIO__Job*)job);
}

void k_io__copyjob_on_connected(void* self, void (*callback)(void*, void*)) {
    KIO__Job_Connect_Connected((KIO__Job*)self, (intptr_t)callback);
}

const char** k_io__copyjob_detailed_error_strings1(void* self, void* reqUrl) {
    libqt_list _arr = KIO__Job_DetailedErrorStrings1((KIO__Job*)self, (QUrl*)reqUrl);
    const libqt_string* _qstr = (libqt_string*)_arr.data.ptr;
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__copyjob_detailed_error_strings1\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
    }
    _ret[_arr.len] = NULL;
    for (size_t i = 0; i < _arr.len; ++i) {
        libqt_string_free((libqt_string*)&_qstr[i]);
    }
    libqt_free(_arr.data.ptr);
    return _ret;
}

const char** k_io__copyjob_detailed_error_strings2(void* self, void* reqUrl, int method) {
    libqt_list _arr = KIO__Job_DetailedErrorStrings2((KIO__Job*)self, (QUrl*)reqUrl, method);
    const libqt_string* _qstr = (libqt_string*)_arr.data.ptr;
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__copyjob_detailed_error_strings2\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
    }
    _ret[_arr.len] = NULL;
    for (size_t i = 0; i < _arr.len; ++i) {
        libqt_string_free((libqt_string*)&_qstr[i]);
    }
    libqt_free(_arr.data.ptr);
    return _ret;
}

void k_io__copyjob_set_ui_delegate(void* self, void* delegate) {
    KJob_SetUiDelegate((KJob*)self, (KJobUiDelegate*)delegate);
}

KJobUiDelegate* k_io__copyjob_ui_delegate(void* self) {
    return KJob_UiDelegate((KJob*)self);
}

int32_t k_io__copyjob_capabilities(void* self) {
    return KJob_Capabilities((KJob*)self);
}

bool k_io__copyjob_is_suspended(void* self) {
    return KJob_IsSuspended((KJob*)self);
}

bool k_io__copyjob_kill(void* self) {
    return KJob_Kill((KJob*)self);
}

bool k_io__copyjob_suspend(void* self) {
    return KJob_Suspend((KJob*)self);
}

bool k_io__copyjob_resume(void* self) {
    return KJob_Resume((KJob*)self);
}

bool k_io__copyjob_exec(void* self) {
    return KJob_Exec((KJob*)self);
}

int32_t k_io__copyjob_error(void* self) {
    return KJob_Error((KJob*)self);
}

const char* k_io__copyjob_error_text(void* self) {
    libqt_string _str = KJob_ErrorText((KJob*)self);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

uintptr_t k_io__copyjob_processed_amount(void* self, int32_t unit) {
    return KJob_ProcessedAmount((KJob*)self, unit);
}

uintptr_t k_io__copyjob_total_amount(void* self, int32_t unit) {
    return KJob_TotalAmount((KJob*)self, unit);
}

uintptr_t k_io__copyjob_percent(void* self) {
    return KJob_Percent((KJob*)self);
}

void k_io__copyjob_set_auto_delete(void* self, bool autodelete) {
    KJob_SetAutoDelete((KJob*)self, autodelete);
}

bool k_io__copyjob_is_auto_delete(void* self) {
    return KJob_IsAutoDelete((KJob*)self);
}

void k_io__copyjob_set_finished_notification_hidden(void* self) {
    KJob_SetFinishedNotificationHidden((KJob*)self);
}

bool k_io__copyjob_is_finished_notification_hidden(void* self) {
    return KJob_IsFinishedNotificationHidden((KJob*)self);
}

bool k_io__copyjob_is_started_with_exec(void* self) {
    return KJob_IsStartedWithExec((KJob*)self);
}

int64_t k_io__copyjob_elapsed_time(void* self) {
    return KJob_ElapsedTime((KJob*)self);
}

void k_io__copyjob_info_message(void* self, void* job, const char* message) {
    KJob_InfoMessage((KJob*)self, (KJob*)job, qstring(message));
}

void k_io__copyjob_on_info_message(void* self, void (*callback)(void*, void*, const char*)) {
    KJob_Connect_InfoMessage((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_warning(void* self, void* job, const char* message) {
    KJob_Warning((KJob*)self, (KJob*)job, qstring(message));
}

void k_io__copyjob_on_warning(void* self, void (*callback)(void*, void*, const char*)) {
    KJob_Connect_Warning((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_total_size(void* self, void* job, uintptr_t size) {
    KJob_TotalSize((KJob*)self, (KJob*)job, size);
}

void k_io__copyjob_on_total_size(void* self, void (*callback)(void*, void*, uintptr_t)) {
    KJob_Connect_TotalSize((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_processed_size(void* self, void* job, uintptr_t size) {
    KJob_ProcessedSize((KJob*)self, (KJob*)job, size);
}

void k_io__copyjob_on_processed_size(void* self, void (*callback)(void*, void*, uintptr_t)) {
    KJob_Connect_ProcessedSize((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_speed(void* self, void* job, uintptr_t speed) {
    KJob_Speed((KJob*)self, (KJob*)job, speed);
}

void k_io__copyjob_on_speed(void* self, void (*callback)(void*, void*, uintptr_t)) {
    KJob_Connect_Speed((KJob*)self, (intptr_t)callback);
}

bool k_io__copyjob_kill1(void* self, int32_t verbosity) {
    return KJob_Kill1((KJob*)self, verbosity);
}

void k_io__copyjob_set_finished_notification_hidden1(void* self, bool hide) {
    KJob_SetFinishedNotificationHidden1((KJob*)self, hide);
}

bool k_io__copyjob_event(void* self, void* event) {
    return QObject_Event((QObject*)self, (QEvent*)event);
}

bool k_io__copyjob_event_filter(void* self, void* watched, void* event) {
    return QObject_EventFilter((QObject*)self, (QObject*)watched, (QEvent*)event);
}

const char* k_io__copyjob_object_name(void* self) {
    libqt_string _str = QObject_ObjectName((QObject*)self);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}

void k_io__copyjob_set_object_name(void* self, const char* name) {
    QObject_SetObjectName((QObject*)self, name);
}

bool k_io__copyjob_is_widget_type(void* self) {
    return QObject_IsWidgetType((QObject*)self);
}

bool k_io__copyjob_is_window_type(void* self) {
    return QObject_IsWindowType((QObject*)self);
}

bool k_io__copyjob_is_quick_item_type(void* self) {
    return QObject_IsQuickItemType((QObject*)self);
}

bool k_io__copyjob_signals_blocked(void* self) {
    return QObject_SignalsBlocked((QObject*)self);
}

bool k_io__copyjob_block_signals(void* self, bool b) {
    return QObject_BlockSignals((QObject*)self, b);
}

QThread* k_io__copyjob_thread(void* self) {
    return QObject_Thread((QObject*)self);
}

bool k_io__copyjob_move_to_thread(void* self, void* thread) {
    return QObject_MoveToThread((QObject*)self, (QThread*)thread);
}

int32_t k_io__copyjob_start_timer(void* self, int interval) {
    return QObject_StartTimer((QObject*)self, interval);
}

int32_t k_io__copyjob_start_timer2(void* self, int64_t time) {
    return QObject_StartTimer2((QObject*)self, time);
}

void k_io__copyjob_kill_timer(void* self, int id) {
    QObject_KillTimer((QObject*)self, id);
}

void k_io__copyjob_kill_timer2(void* self, int32_t id) {
    QObject_KillTimer2((QObject*)self, id);
}

libqt_list /* of QObject* */ k_io__copyjob_children(void* self) {
    libqt_list _arr = QObject_Children((QObject*)self);
    return _arr;
}

void k_io__copyjob_set_parent(void* self, void* parent) {
    QObject_SetParent((QObject*)self, (QObject*)parent);
}

void k_io__copyjob_install_event_filter(void* self, void* filterObj) {
    QObject_InstallEventFilter((QObject*)self, (QObject*)filterObj);
}

void k_io__copyjob_remove_event_filter(void* self, void* obj) {
    QObject_RemoveEventFilter((QObject*)self, (QObject*)obj);
}

QMetaObject__Connection* k_io__copyjob_connect(void* sender, const char* signal, void* receiver, const char* member) {
    return QObject_Connect((QObject*)sender, signal, (QObject*)receiver, member);
}

QMetaObject__Connection* k_io__copyjob_connect2(void* sender, void* signal, void* receiver, void* method) {
    return QObject_Connect2((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method);
}

QMetaObject__Connection* k_io__copyjob_connect3(void* self, void* sender, const char* signal, const char* member) {
    return QObject_Connect3((QObject*)self, (QObject*)sender, signal, member);
}

bool k_io__copyjob_disconnect(void* sender, const char* signal, void* receiver, const char* member) {
    return QObject_Disconnect((QObject*)sender, signal, (QObject*)receiver, member);
}

bool k_io__copyjob_disconnect2(void* sender, void* signal, void* receiver, void* member) {
    return QObject_Disconnect2((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)member);
}

bool k_io__copyjob_disconnect3(void* self) {
    return QObject_Disconnect3((QObject*)self);
}

bool k_io__copyjob_disconnect4(void* self, void* receiver) {
    return QObject_Disconnect4((QObject*)self, (QObject*)receiver);
}

bool k_io__copyjob_disconnect5(void* param1) {
    return QObject_Disconnect5((QMetaObject__Connection*)param1);
}

void k_io__copyjob_dump_object_tree(void* self) {
    QObject_DumpObjectTree((QObject*)self);
}

void k_io__copyjob_dump_object_info(void* self) {
    QObject_DumpObjectInfo((QObject*)self);
}

bool k_io__copyjob_set_property(void* self, const char* name, void* value) {
    return QObject_SetProperty((QObject*)self, name, (QVariant*)value);
}

QVariant* k_io__copyjob_property(void* self, const char* name) {
    return QObject_Property((QObject*)self, name);
}

const char** k_io__copyjob_dynamic_property_names(void* self) {
    libqt_list _arr = QObject_DynamicPropertyNames((QObject*)self);
    const libqt_string* _qstr = (libqt_string*)_arr.data.ptr;
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__copyjob_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
    }
    _ret[_arr.len] = NULL;
    for (size_t i = 0; i < _arr.len; ++i) {
        libqt_string_free((libqt_string*)&_qstr[i]);
    }
    libqt_free(_arr.data.ptr);
    return _ret;
}

QBindingStorage* k_io__copyjob_binding_storage(void* self) {
    return QObject_BindingStorage((QObject*)self);
}

const QBindingStorage* k_io__copyjob_binding_storage2(void* self) {
    return QObject_BindingStorage2((QObject*)self);
}

void k_io__copyjob_destroyed(void* self) {
    QObject_Destroyed((QObject*)self);
}

void k_io__copyjob_on_destroyed(void* self, void (*callback)(void*)) {
    QObject_Connect_Destroyed((QObject*)self, (intptr_t)callback);
}

QObject* k_io__copyjob_parent(void* self) {
    return QObject_Parent((QObject*)self);
}

bool k_io__copyjob_inherits(void* self, const char* classname) {
    return QObject_Inherits((QObject*)self, classname);
}

void k_io__copyjob_delete_later(void* self) {
    QObject_DeleteLater((QObject*)self);
}

int32_t k_io__copyjob_start_timer22(void* self, int interval, int32_t timerType) {
    return QObject_StartTimer22((QObject*)self, interval, timerType);
}

int32_t k_io__copyjob_start_timer23(void* self, int64_t time, int32_t timerType) {
    return QObject_StartTimer23((QObject*)self, time, timerType);
}

QMetaObject__Connection* k_io__copyjob_connect5(void* sender, const char* signal, void* receiver, const char* member, int32_t param5) {
    return QObject_Connect5((QObject*)sender, signal, (QObject*)receiver, member, param5);
}

QMetaObject__Connection* k_io__copyjob_connect52(void* sender, void* signal, void* receiver, void* method, int32_t type) {
    return QObject_Connect52((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method, type);
}

QMetaObject__Connection* k_io__copyjob_connect4(void* self, void* sender, const char* signal, const char* member, int32_t type) {
    return QObject_Connect4((QObject*)self, (QObject*)sender, signal, member, type);
}

bool k_io__copyjob_disconnect1(void* self, const char* signal) {
    return QObject_Disconnect1((QObject*)self, signal);
}

bool k_io__copyjob_disconnect22(void* self, const char* signal, void* receiver) {
    return QObject_Disconnect22((QObject*)self, signal, (QObject*)receiver);
}

bool k_io__copyjob_disconnect32(void* self, const char* signal, void* receiver, const char* member) {
    return QObject_Disconnect32((QObject*)self, signal, (QObject*)receiver, member);
}

bool k_io__copyjob_disconnect23(void* self, void* receiver, const char* member) {
    return QObject_Disconnect23((QObject*)self, (QObject*)receiver, member);
}

void k_io__copyjob_destroyed1(void* self, void* param1) {
    QObject_Destroyed1((QObject*)self, (QObject*)param1);
}

void k_io__copyjob_on_destroyed1(void* self, void (*callback)(void*, void*)) {
    QObject_Connect_Destroyed1((QObject*)self, (intptr_t)callback);
}

void k_io__copyjob_on_finished(void* self, void (*callback)(void*, void*)) {
    KJob_Connect_Finished((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_on_suspended(void* self, void (*callback)(void*, void*)) {
    KJob_Connect_Suspended((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_on_resumed(void* self, void (*callback)(void*, void*)) {
    KJob_Connect_Resumed((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_on_result(void* self, void (*callback)(void*, void*)) {
    KJob_Connect_Result((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_on_total_amount_changed(void* self, void (*callback)(void*, void*, int32_t, uintptr_t)) {
    KJob_Connect_TotalAmountChanged((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_on_processed_amount_changed(void* self, void (*callback)(void*, void*, int32_t, uintptr_t)) {
    KJob_Connect_ProcessedAmountChanged((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_on_percent_changed(void* self, void (*callback)(void*, void*, uintptr_t)) {
    KJob_Connect_PercentChanged((KJob*)self, (intptr_t)callback);
}

void k_io__copyjob_on_object_name_changed(void* self, void (*callback)(void*, const char*)) {
    QObject_Connect_ObjectNameChanged((QObject*)self, (intptr_t)callback);
}

void k_io__copyjob_delete(void* self) {
    KIO__CopyJob_Delete((KIO__CopyJob*)(self));
}

KIO__CopyJob* k_io_copy(void* param1, void* param2, int32_t param3) {
    return KIO_Copy((QUrl*)param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_copy_as(void* param1, void* param2, int32_t param3) {
    return KIO_CopyAs((QUrl*)param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_copy2(libqt_list /* of QUrl* */ param1, void* param2, int32_t param3) {
    return KIO_Copy2(param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_move(void* param1, void* param2, int32_t param3) {
    return KIO_Move((QUrl*)param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_move_as(void* param1, void* param2, int32_t param3) {
    return KIO_MoveAs((QUrl*)param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_move2(libqt_list /* of QUrl* */ param1, void* param2, int32_t param3) {
    return KIO_Move2(param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_link(void* param1, void* param2, int32_t param3) {
    return KIO_Link((QUrl*)param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_link2(libqt_list /* of QUrl* */ param1, void* param2, int32_t param3) {
    return KIO_Link2(param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_link_as(void* param1, void* param2, int32_t param3) {
    return KIO_LinkAs((QUrl*)param1, (QUrl*)param2, param3);
}

KIO__CopyJob* k_io_trash(void* param1, int32_t param2) {
    return KIO_Trash((QUrl*)param1, param2);
}

KIO__CopyJob* k_io_trash2(libqt_list /* of QUrl* */ param1, int32_t param2) {
    return KIO_Trash2(param1, param2);
}
