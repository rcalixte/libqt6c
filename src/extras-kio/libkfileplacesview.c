...
void k_fileplacesview_on_indexes_moved(void* self, void (*callback)(void*, libqt_list /* of QModelIndex* */)) {
...
QWidget* k_fileplacesview_find(uintptr_t param1) {
...
const char** k_fileplacesview_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_fileplacesview_dynamic_property_names\n");
        abort();
    }
