...
int32_t k_io__workerbase_ssl_error(void* self, libqt_map /* of const char* to QVariant* */ sslData) {
    ...
    if (sslData_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in k_io__workerbase_ssl_error\n");
        abort();
    }
    sslData_ret.values = (QVariant**)malloc(sslData_ret.len * sizeof(QVariant*));
    if (sslData_ret.values == NULL) {
        free(sslData_ret.keys);
        fprintf(stderr, "Failed to allocate memory for map values in k_io__workerbase_ssl_error\n");
        abort();
    }
...
KIO__WorkerResult* k_io__workerbase_super_open_connection(void* self) {
    return KIO__WorkerBase_SuperOpenConnection((KIO__WorkerBase*)self);
}
...
void k_io__workerbase_super_worker_status2(void* self) {
    KIO__WorkerBase_SuperWorkerStatus2((KIO__WorkerBase*)self);
}
...
void k_io__workerbase_super_reparse_configuration(void* self) {
    KIO__WorkerBase_SuperReparseConfiguration((KIO__WorkerBase*)self);
}
