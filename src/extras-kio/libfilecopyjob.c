...
const char* k_io__filecopyjob_tr(const char* s) {
    libqt_string _str = KIO__FileCopyJob_Tr(s);
...
const char* k_io__filecopyjob_tr2(const char* s, const char* c) {
    libqt_string _str = KIO__FileCopyJob_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_io__filecopyjob_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KIO__FileCopyJob_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** k_io__filecopyjob_detailed_error_strings(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__filecopyjob_detailed_error_strings");
        abort();
    }
...
const char** k_io__filecopyjob_detailed_error_strings1(void* self, void* reqUrl) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__filecopyjob_detailed_error_strings1");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__filecopyjob_detailed_error_strings2(void* self, void* reqUrl, int method) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__filecopyjob_detailed_error_strings2");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__filecopyjob_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__filecopyjob_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
