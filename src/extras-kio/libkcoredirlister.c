...
const char* k_coredirlister_tr(const char* s) {
    libqt_string _str = KCoreDirLister_Tr(s);
...
void k_coredirlister_set_mime_filter(void* self, const char* mimeList[]) {
    ...
    if (mimeList_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_coredirlister_set_mime_filter");
        abort();
    }
...
void k_coredirlister_set_mime_exclude_filter(void* self, const char* mimeList[]) {
    ...
    libqt_string* mimeList_qstr = (libqt_string*)malloc(mimeList_len * sizeof(libqt_string));
    if (mimeList_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_coredirlister_set_mime_exclude_filter");
        abort();
    }
    for (size_t i = 0; i < mimeList_len; ++i) {
        mimeList_qstr[i] = qstring(mimeList[i]);
...
const char** k_coredirlister_mime_filters(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_coredirlister_mime_filters");
        abort();
    }
...
const char* k_coredirlister_tr2(const char* s, const char* c) {
    libqt_string _str = KCoreDirLister_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_coredirlister_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KCoreDirLister_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** k_coredirlister_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_coredirlister_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
QMetaObject__Connection* k_coredirlister_connect5(void* sender, void* signal, void* receiver, void* method, int32_t typeVal) {
    return QObject_Connect5((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method, typeVal);
}

QMetaObject__Connection* k_coredirlister_connect4(void* self, void* sender, const char* signal, const char* member, int32_t typeVal) {
