...
const char** k_protocolinfo_protocols() {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_protocolinfo_protocols");
        abort();
    }
...
const char** k_protocolinfo_capabilities(const char* protocol) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_protocolinfo_capabilities");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_protocolinfo_archive_mimetypes(const char* protocol) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_protocolinfo_archive_mimetypes");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
