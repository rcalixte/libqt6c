...
const QMetaObject* k_io__statjob_meta_object(void* self) {
    return KIO__StatJob_MetaObject((KIO__StatJob*)self);
}

void* k_io__statjob_metacast(void* self, const char* param1) {
    return KIO__StatJob_Metacast((KIO__StatJob*)self, param1);
}

int32_t k_io__statjob_metacall(void* self, int32_t param1, int param2, void* param3) {
    return KIO__StatJob_Metacall((KIO__StatJob*)self, param1, param2, param3);
}
...
void k_io__statjob_set_side(void* self, int32_t side) {
    KIO__StatJob_SetSide((KIO__StatJob*)self, side);
}

void k_io__statjob_set_details(void* self, int32_t details) {
    KIO__StatJob_SetDetails((KIO__StatJob*)self, details);
}

const KIO__UDSEntry* k_io__statjob_stat_result(void* self) {
    return KIO__StatJob_StatResult((KIO__StatJob*)self);
}

QUrl* k_io__statjob_most_local_url(void* self) {
    return KIO__StatJob_MostLocalUrl((KIO__StatJob*)self);
}

void k_io__statjob_redirection(void* self, void* job, void* url) {
    KIO__StatJob_Redirection((KIO__StatJob*)self, (KIO__Job*)job, (QUrl*)url);
}

void k_io__statjob_on_redirection(void* self, void (*callback)(void*, void*, void*)) {
    KIO__StatJob_Connect_Redirection((KIO__StatJob*)self, (intptr_t)callback);
}

void k_io__statjob_permanent_redirection(void* self, void* job, void* fromUrl, void* toUrl) {
    KIO__StatJob_PermanentRedirection((KIO__StatJob*)self, (KIO__Job*)job, (QUrl*)fromUrl, (QUrl*)toUrl);
}

void k_io__statjob_on_permanent_redirection(void* self, void (*callback)(void*, void*, void*, void*)) {
    KIO__StatJob_Connect_PermanentRedirection((KIO__StatJob*)self, (intptr_t)callback);
}
...
const char** k_io__statjob_detailed_error_strings(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__statjob_detailed_error_strings\n");
        abort();
    }
...
void k_io__statjob_add_meta_data2(void* self, libqt_map /* of const char* to const char* */ values) {
    ...
    if (values_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in k_io__statjob_add_meta_data2\n");
        abort();
    }
    values_ret.values = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.values == NULL) {
        free(values_ret.keys);
        fprintf(stderr, "Failed to allocate memory for map values in k_io__statjob_add_meta_data2\n");
        abort();
    }
...
void k_io__statjob_merge_meta_data(void* self, libqt_map /* of const char* to const char* */ values) {
    ...
    values_ret.keys = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in k_io__statjob_merge_meta_data\n");
        abort();
    }
    values_ret.values = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.values == NULL) {
        free(values_ret.keys);
        fprintf(stderr, "Failed to allocate memory for map values in k_io__statjob_merge_meta_data\n");
        abort();
    }
    const char** values_karr = (const char**)values.keys;
    libqt_string* values_kdest = (libqt_string*)values_ret.keys;
...
const char** k_io__statjob_detailed_error_strings1(void* self, void* reqUrl) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__statjob_detailed_error_strings1\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__statjob_detailed_error_strings2(void* self, void* reqUrl, int method) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__statjob_detailed_error_strings2\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__statjob_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__statjob_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
