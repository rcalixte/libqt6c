...
const char* k_shellcompletion_tr(const char* s) {
    libqt_string _str = KShellCompletion_Tr(s);
...
void k_shellcompletion_post_process_matches(void* self, const char* matches[]) {
    ...
    if (matches_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_post_process_matches");
        abort();
    }
...
const char* k_shellcompletion_tr2(const char* s, const char* c) {
    libqt_string _str = KShellCompletion_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_shellcompletion_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KShellCompletion_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
void k_shellcompletion_set_mime_type_filters(void* self, const char* mimeTypes[]) {
    ...
    if (mimeTypes_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_set_mime_type_filters");
        abort();
    }
...
const char** k_shellcompletion_mime_type_filters(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_mime_type_filters");
        abort();
    }
...
const char** k_shellcompletion_substring_completion(void* self, const char* stringVal) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_substring_completion");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_shellcompletion_items(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_items");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_shellcompletion_all_matches(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_all_matches");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_shellcompletion_all_matches2(void* self, const char* stringVal) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_all_matches2");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
void k_shellcompletion_insert_items(void* self, const char* items[]) {
    ...
    if (items_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_insert_items");
        abort();
    }
...
void k_shellcompletion_matches(void* self, const char* matchlist[]) {
    ...
    if (matchlist_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_matches");
        abort();
    }
...
const char** k_shellcompletion_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
QMetaObject__Connection* k_shellcompletion_connect5(void* sender, void* signal, void* receiver, void* method, int32_t typeVal) {
    return QObject_Connect5((QObject*)sender, (QMetaMethod*)signal, (QObject*)receiver, (QMetaMethod*)method, typeVal);
}

QMetaObject__Connection* k_shellcompletion_connect4(void* self, void* sender, const char* signal, const char* member, int32_t typeVal) {
...
void k_shellcompletion_set_items(void* self, const char* itemList[]) {
    ...
    if (itemList_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_shellcompletion_set_items");
        abort();
    }
