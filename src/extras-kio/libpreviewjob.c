...
KIO__PreviewJob* k_io__previewjob_new2(void* items, void* size, const char* enabledPlugins[]) {
    ...
    if (enabledPlugins_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__previewjob_new2");
        abort();
    }
...
const char* k_io__previewjob_tr(const char* s) {
    libqt_string _str = KIO__PreviewJob_Tr(s);
...
void k_io__previewjob_set_scale_type(void* self, int32_t typeVal) {
    KIO__PreviewJob_SetScaleType((KIO__PreviewJob*)self, typeVal);
}
...
const char** k_io__previewjob_available_plugins() {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__previewjob_available_plugins");
        abort();
    }
...
const char** k_io__previewjob_default_plugins() {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__previewjob_default_plugins");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__previewjob_supported_mime_types() {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__previewjob_supported_mime_types");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char* k_io__previewjob_tr2(const char* s, const char* c) {
    libqt_string _str = KIO__PreviewJob_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_io__previewjob_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KIO__PreviewJob_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** k_io__previewjob_detailed_error_strings(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__previewjob_detailed_error_strings");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__previewjob_detailed_error_strings1(void* self, void* reqUrl) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__previewjob_detailed_error_strings1");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__previewjob_detailed_error_strings2(void* self, void* reqUrl, int method) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__previewjob_detailed_error_strings2");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__previewjob_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__previewjob_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
KIO__PreviewJob* k_io_file_preview(void* param1, void* param2, const char* param3[]) {
    ...
    if (param3_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io_file_preview");
        abort();
    }
