...
const char** k_dirmodel_mime_types(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_dirmodel_mime_types\n");
        abort();
    }
...
libqt_map /* of int to char* */ k_dirmodel_role_names(void* self) {
    ...
    if (_ret_values == NULL) {
        fprintf(stderr, "Failed to allocate memory for map string values in k_dirmodel_role_names\n");
        abort();
    }
...
libqt_list /* of QUrl* */ k_dirmodel_simplified_url_list(libqt_list /* of QUrl* */ urls) {
...
const char** k_dirmodel_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_dirmodel_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
bool k_dirmodel_set_item_data(void* self, void* index, libqt_map /* of int to QVariant* */ roles) {
    ...
    if (roles_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in k_dirmodel_set_item_data\n");
        abort();
    }
    roles_ret.values = (QVariant**)malloc(roles_ret.len * sizeof(QVariant*));
    if (roles_ret.values == NULL) {
        free(roles_ret.keys);
        fprintf(stderr, "Failed to allocate memory for map values in k_dirmodel_set_item_data\n");
        abort();
    }
