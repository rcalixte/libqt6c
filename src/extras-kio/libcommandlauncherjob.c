...
KIO__CommandLauncherJob* k_io__commandlauncherjob_new2(const char* executable, const char* args[]) {
    ...
    if (args_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__commandlauncherjob_new2");
        abort();
    }
...
KIO__CommandLauncherJob* k_io__commandlauncherjob_new4(const char* executable, const char* args[], void* parent) {
    ...
    libqt_string* args_qstr = (libqt_string*)malloc(args_len * sizeof(libqt_string));
    if (args_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__commandlauncherjob_new4");
        abort();
    }
    for (size_t i = 0; i < args_len; ++i) {
        args_qstr[i] = qstring(args[i]);
...
const char** k_io__commandlauncherjob_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_io__commandlauncherjob_dynamic_property_names");
        abort();
    }
