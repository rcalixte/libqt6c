...
const QMetaObject* k_io__mkpathjob_meta_object(void* self) {
    return KIO__MkpathJob_MetaObject((KIO__MkpathJob*)self);
}

void* k_io__mkpathjob_metacast(void* self, const char* param1) {
    return KIO__MkpathJob_Metacast((KIO__MkpathJob*)self, param1);
}

int32_t k_io__mkpathjob_metacall(void* self, int32_t param1, int param2, void* param3) {
    return KIO__MkpathJob_Metacall((KIO__MkpathJob*)self, param1, param2, param3);
}
...
void k_io__mkpathjob_directory_created(void* self, void* url) {
    KIO__MkpathJob_DirectoryCreated((KIO__MkpathJob*)self, (QUrl*)url);
}

void k_io__mkpathjob_on_directory_created(void* self, void (*callback)(void*, void*)) {
    KIO__MkpathJob_Connect_DirectoryCreated((KIO__MkpathJob*)self, (intptr_t)callback);
}
...
const char** k_io__mkpathjob_detailed_error_strings(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__mkpathjob_detailed_error_strings\n");
        abort();
    }
...
void k_io__mkpathjob_add_meta_data2(void* self, libqt_map /* of const char* to const char* */ values) {
    ...
    if (values_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in k_io__mkpathjob_add_meta_data2\n");
        abort();
    }
    values_ret.values = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.values == NULL) {
        free(values_ret.keys);
        fprintf(stderr, "Failed to allocate memory for map values in k_io__mkpathjob_add_meta_data2\n");
        abort();
    }
...
void k_io__mkpathjob_merge_meta_data(void* self, libqt_map /* of const char* to const char* */ values) {
    ...
    values_ret.keys = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.keys == NULL) {
        fprintf(stderr, "Failed to allocate memory for map keys in k_io__mkpathjob_merge_meta_data\n");
        abort();
    }
    values_ret.values = (libqt_string*)malloc(values_ret.len * sizeof(libqt_string));
    if (values_ret.values == NULL) {
        free(values_ret.keys);
        fprintf(stderr, "Failed to allocate memory for map values in k_io__mkpathjob_merge_meta_data\n");
        abort();
    }
    const char** values_karr = (const char**)values.keys;
    libqt_string* values_kdest = (libqt_string*)values_ret.keys;
...
const char** k_io__mkpathjob_detailed_error_strings1(void* self, void* reqUrl) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__mkpathjob_detailed_error_strings1\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__mkpathjob_detailed_error_strings2(void* self, void* reqUrl, int method) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__mkpathjob_detailed_error_strings2\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_io__mkpathjob_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in k_io__mkpathjob_dynamic_property_names\n");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
