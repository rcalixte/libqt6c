...
const char** q_designermembersheetextension_parameter_types(void* self, int index) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_designermembersheetextension_parameter_types");
        abort();
    }
...
const char** q_designermembersheetextension_parameter_names(void* self, int index) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in q_designermembersheetextension_parameter_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
