...
const char* k_nscore__question_tr(const char* s) {
    libqt_string _str = KNSCore__Question_Tr(s);
...
void k_nscore__question_set_list(void* self, const char* newList[static 1]) {
    ...
    if (newList_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__question_set_list");
        abort();
    }
...
const char** k_nscore__question_list(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__question_list");
        abort();
    }
...
const char* k_nscore__question_tr2(const char* s, const char* c) {
    libqt_string _str = KNSCore__Question_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_nscore__question_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KNSCore__Question_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** k_nscore__question_dynamic_property_names(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__question_dynamic_property_names");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
