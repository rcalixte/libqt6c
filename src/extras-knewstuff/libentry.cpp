...
void KNSCore__Entry_SetTags(KNSCore__Entry* self, const libqt_list /* of libqt_string */ tags) {
    ...
    libqt_string* tags_arr = static_cast<libqt_string*>(tags.data.ptr);
...
void KNSCore__Entry_SetEntryType(KNSCore__Entry* self, int type) {
    self->setEntryType(static_cast<KNSCore::Entry::EntryType>(type));
}
...
void KNSCore__Entry_SetPreviewUrl2(KNSCore__Entry* self, const libqt_string url, int type) {
    QString url_QString = QString::fromUtf8(url.data, url.len);
    self->setPreviewUrl(url_QString, static_cast<KNSCore::Entry::PreviewType>(type));
}

libqt_string KNSCore__Entry_PreviewUrl1(const KNSCore__Entry* self, int type) {
    QString _ret = self->previewUrl(static_cast<KNSCore::Entry::PreviewType>(type));
    // Convert QString from UTF-16 in C++ RAII memory to UTF-8 in manually-managed C memory
    QByteArray _b = _ret.toUtf8();
    libqt_string _str;
    _str.len = _b.length();
...
QImage* KNSCore__Entry_PreviewImage1(const KNSCore__Entry* self, int type) {
    return new QImage(self->previewImage(static_cast<KNSCore::Entry::PreviewType>(type)));
}

void KNSCore__Entry_SetPreviewImage2(KNSCore__Entry* self, const QImage* image, int type) {
    self->setPreviewImage(*image, static_cast<KNSCore::Entry::PreviewType>(type));
}
...
libqt_list /* of libqt_string */ KNSCore__Entry__DownloadLinkInformation_Tags(const KNSCore__Entry__DownloadLinkInformation* self) {
    ...
    tags_out.data.ptr = static_cast<void*>(tags_arr);
    return tags_out;
}
