...
void k_nscore__entry_set_installed_files(void* self, const char* files[]) {
    ...
    if (files_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__entry_set_installed_files");
        abort();
    }
...
const char** k_nscore__entry_installed_files(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__entry_installed_files");
        abort();
    }
...
const char** k_nscore__entry_uninstalled_files(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__entry_uninstalled_files");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
const char** k_nscore__entry_tags(void* self) {
    ...
    const char** _ret = (const char**)malloc((_arr.len + 1) * sizeof(const char*));
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__entry_tags");
        abort();
    }
    for (size_t i = 0; i < _arr.len; ++i) {
        _ret[i] = qstring_to_char(_qstr[i]);
...
void k_nscore__entry_set_tags(void* self, const char* tags[]) {
    ...
    if (tags_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__entry_set_tags");
        abort();
    }
...
const char** k_nscore__entry__downloadlinkinformation_tags(void* self) {
    ...
    if (tags_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__entry__downloadlinkinformation_tags");
        abort();
    }
...
void k_nscore__entry__downloadlinkinformation_set_tags(void* self, const char* tags[]) {
    ...
    libqt_string* tags_qstr = (libqt_string*)malloc(tags_len * sizeof(libqt_string));
    if (tags_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_nscore__entry__downloadlinkinformation_set_tags");
        abort();
    }
    for (size_t i = 0; i < tags_len; ++i) {
        tags_qstr[i] = qstring(tags[i]);
