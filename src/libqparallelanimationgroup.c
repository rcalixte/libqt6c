...
int32_t q_parallelanimationgroup_start_timer2(void* self, int64_t time) {
...
const char** q_parallelanimationgroup_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Failed to allocate memory for string list in q_parallelanimationgroup_dynamic_property_names\n");
        abort();
    }
...
int32_t q_parallelanimationgroup_start_timer23(void* self, int64_t time, int32_t timerType) {
    return QObject_StartTimer23((QObject*)self, time, timerType);
}

QMetaObject__Connection* q_parallelanimationgroup_connect5(void* sender, void* signal, void* receiver, void* method, int32_t type) {
