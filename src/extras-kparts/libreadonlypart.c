...
const char* k_parts__readonlypart_tr(const char* s) {
    libqt_string _str = KParts__ReadOnlyPart_Tr(s);
...
const char* k_parts__readonlypart_tr2(const char* s, const char* c) {
    libqt_string _str = KParts__ReadOnlyPart_Tr2(s, c);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char* k_parts__readonlypart_tr3(const char* s, const char* c, int n) {
    libqt_string _str = KParts__ReadOnlyPart_Tr3(s, c, n);
    char* _ret = qstring_to_char(_str);
    libqt_string_free(&_str);
    return _ret;
}
...
const char** k_parts__readonlypart_dynamic_property_names(void* self) {
    ...
    if (_ret == NULL) {
        fprintf(stderr, "Memory allocation failed in k_parts__readonlypart_dynamic_property_names");
        abort();
    }
...
const char* k_parts__readonlypart_find_most_recent_x_m_l_file(const char* files[], const char* doc) {
    size_t files_len = libqt_strv_length(files);
    libqt_string* files_qstr = (libqt_string*)malloc(files_len * sizeof(libqt_string));
    if (files_qstr == NULL) {
        fprintf(stderr, "Memory allocation failed in k_parts__readonlypart_find_most_recent_x_m_l_file");
        abort();
    }
