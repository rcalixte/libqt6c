...
func renderProperties(properties []UiProperty, ret *strings.Builder, targetName, targetClass, parentClass string, warningContext WarningContext) error {
	...
	for _, prop := range properties {
		...
		if prop.Name == "geometry" {
			...
			ret.WriteString("QLocale* locale_" + targetName + " = q_locale_new4(QLOCALE_LANGUAGE_" + strings.ToUpper(prop.LocaleVal.Language) + ", QLOCALE_COUNTRY_" + strings.ToUpper(prop.LocaleVal.Country) + ");\n")
	...
	for _, attr := range w.Attributes {
		if parentClass == "QTabWidget" && attr.Name == "title" {
			if attr.StringVal.Notr {
				ret.WriteString("\nq_tabwidget_add_tab(" + parentName + ", ui->" + w.Name + ", " + generateString(attr.StringVal) + ");\n")
			} else {
...
func generate(goGenerateArgs string, flagExtraOps UiFlagOptions, u UiFile) ([]byte, error) {
	...
	cMethod := strings.TrimPrefix(cMethodName(uClass), "_")
	...
	wClass := u.Widget.Class
	if cw, ok := CustomWidgets[u.Widget.Class]; ok && !(ExtendedFlag && isExtendedClass(u.Widget.Class)) {
		wClass = cw
	}
	wClassC := cClassMethodPrefix(strings.ReplaceAll(wClass, "::", "__"))
...
static void cleanup_` + cMethod + "_ui(const " + uClass + `Ui* ui) {
    if (` + wClassC + `_parent_widget(ui->` + u.Widget.Name + `) == NULL)
        ` + wClassC + `_delete(ui->` + u.Widget.Name + `);
}
`)
...
static void retranslate_` + cMethod + "_ui(const " + uClass + `Ui* ui) {
...
static void initialize_` + cMethod + `_ui(` + uClass + `Ui* ui, void* parent) {`)
	...
	for i := 1; i < len(u.TabStops.TabStops); i++ {
		...
		ret.WriteString("q_widget_set_tab_order(ui->" + previous + ", ui->" + current + ");\n")
	}
