...
// A recognized extended class is currently defined as a class name where the
...
func walkWidget(w *UiWidget) {
	if w.Name != "" {
		if _, ok := cReservedWord[w.Name]; ok {
			notifyRename(w.Name)
			w.Name = "_" + w.Name
		}
	} else {
		w.Name = cClassMethodPrefix(w.Class)[1:] + strconv.Itoa(SanitizeObjectCounter)
	...
	for i := range w.Actions {
		if _, ok := cReservedWord[w.Actions[i].Name]; ok {
...
func walkLayout(l *UiLayout) {
	if l.Name != "" {
		if _, ok := cReservedWord[l.Name]; ok {
			notifyRename(l.Name)
			l.Name = "_" + l.Name
		}
	} else {
		l.Name = cClassMethodPrefix(l.Class)[1:] + strconv.Itoa(SanitizeObjectCounter)
	...
	for i := range l.Items {
		...
		if l.Items[i].Spacer != nil && l.Items[i].Spacer.Name != "" {
			if _, ok := cReservedWord[l.Items[i].Spacer.Name]; ok {
			...
			l.Items[i].Spacer.Name = "_" + cClassName("spaceritem") + strconv.Itoa(SanitizeObjectCounter)
...
func cClassMethodPrefix(name string) string {
	if len(name) < 2 {
