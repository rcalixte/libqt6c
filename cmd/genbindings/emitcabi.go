...
func (p CppParameter) RenderTypeCabi(isSlot bool) string {
	if p.ParameterType == "QString" || p.ParameterType == "QByteArray" {
		...
		return "libqt_list " + cppComment("set of "+inner.RenderTypeCabi(isSlot))
...
func emitCABI2CppForwarding(p CppParameter, indent, currentClass string, isSlot bool) (preamble, forwarding string) {
	...
	if p.ParameterType == "QString" {
		...
		if isSlot && lType == "int" {
			...
			preamble += indent + "for (int* ptr = " + p.ParameterName + "; *ptr != 0; ++ptr) {\n"
...
func emitAssignCppToCabi(assignExpression string, p CppParameter, rvalue string) string {
	...
	if p.Void() {
		...
		if t.ParameterType == "QString" || t.ParameterType == "QByteArray" {
			...
			afterCall += emitAssignCppToCabi(indent+"\t"+namePrefix+"_arr["+namePrefix+"_sctr++] = ", t, namePrefix+"_itr.next()")
			afterCall += indent + "}\n"
