...
func ProcessLibraries(clangBin, outDir, extraLibsDir string) {
	...
	bsdBlockedMap := map[string]bool{
		"foss-extras-kwindowsystem/libkkeyserver.h":        true,
		"foss-extras-kwindowsystem/libkselectionowner.h":   true,
		"foss-extras-kwindowsystem/libkselectionwatcher.h": true,
		"foss-extras-kwindowsystem/libkstartupinfo.h":      true,
		"foss-extras-kwindowsystem/libkxmessages.h":        true,
	}
	qt6cHeader := "// THIS FILE IS AUTOMATICALLY GENERATED\n#pragma once\n\n#include \"qtlibc.h\"\n#include \"libqttypedefs.h\"\n#include \"threading/libqt6cthreading.h\"\n"
	for _, k := range headerList {
		isBlocked := bsdBlockedMap[k]
		if isBlocked {
			qt6cHeader += "#if !defined(__DragonFly__) && !defined(__FreeBSD__) && !defined(__NetBSD__) && !defined(__OpenBSD__)\n"
		}
