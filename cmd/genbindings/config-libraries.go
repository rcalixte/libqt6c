...
func ProcessLibraries(clangBin, outDir, extraLibsDir string) {
	...
	builder.WriteString("// THIS FILE IS AUTOMATICALLY GENERATED\n\n" +
		"#pragma once\n" +
		"#ifndef LIBQTTYPEDEFS_H\n" +
		"#define LIBQTTYPEDEFS_H\n\n" +
		"#include <stddef.h>\n" +
		"#include <stdint.h>\n\n")
	...
	for i := range typedefs {
		builder.WriteString(typedefs[i] + "\n")
	}
	...
	for i := range sortedFunctions {
		fTypedef := allqtfuncdefs[sortedFunctions[i]]
		builder.WriteString("typedef " + fTypedef)
	}
	builder.WriteString("\n#endif\n")

	err := os.WriteFile(outputName, []byte(builder.String()), 0644)
	if err != nil {
		panic(err)
	}

	typesFile := filepath.Join(outDir, "src", "libqttypedefs.h")
	err = os.WriteFile(typesFile, []byte(builder.String()), 0644)
	if err != nil {
		panic(err)
	}
	...
	for i := range allBatches {
		allFiles = append(allFiles, allBatches[i].files...)
		maps.Copy(allCopies, allBatches[i].copies)
	}
	...
	err = os.WriteFile(filepath.Join(outDir, "include", "libqt6c.h"), []byte(builder.String()), 0644)
	if err != nil {
		panic(err)
	}
	...
	builder.WriteString("<!-- This file is automatically generated -->\n\n" +
		...
		"These are used to either upcast to a class object type of a secondary parent class or " +
		"downcast to a base class object from the same secondary parent class.\n" +
		"These are required when passing a base or inherited class object to a function " +
		"that expects a secondary parent class or when a function returns a secondary parent class object and downcasting is desired.\n" +
		"All types inheriting from these base classes also inherit the casting methods where possible.\n\n" +
		`<table align="center"` +
		"<tr><th>Class</th><th>Direct Inheritance</th><th>Secondary Inheritance(s)</th></tr>\n")
	upcastKeys := []string{}
	for className, classInheritance := range inheritanceMap {
		if len(classInheritance["secondary"]) == 0 {
			continue
		}
		upcastKeys = append(upcastKeys, className)
		sort.Strings(classInheritance["secondary"])
	}
	sort.Strings(upcastKeys)
	for _, className := range upcastKeys {
		classInheritance := inheritanceMap[className]
		if len(classInheritance["secondary"]) == 0 {
			continue
		}
		builder.WriteString("  <tr><td><code>" + className + "</code></td>")
		builder.WriteString("<td><code>" + classInheritance["direct"][0] + "</code></td>")
		builder.WriteString("<td><code>" + strings.Join(classInheritance["secondary"], "</code>, <code>") + "</code></td></tr>\n")
	}
	builder.WriteString("</table>\n")
	err = os.WriteFile(filepath.Join(outDir, "INHERITANCE.md"), []byte(builder.String()), 0644)
	if err != nil {
		panic(err)
	}
}
