	...
	for _, node := range inner {
		...
		case "FieldDecl":
			...
			if typobj, ok := node["type"].(map[string]any); ok {
				...
				if qualType != "" {
					fieldType = parseSingleTypeString(qualType, ret.ClassName)
					...
					if strings.HasPrefix(ret.ClassName, "QOpenGL") && strings.Contains(ret.ClassName, "Deprecated") && fieldName == "AreTexturesResident" {
						// Qt 6: deprecated function with currently unsupported function parameter type
						continue
					}
					...
					if strings.HasPrefix(ret.ClassName, "QOpenGL") && strings.HasSuffix(ret.ClassName, "Functions") &&
						...
						// Qt 6: currently unsupported function parameter type
						continue
					}
