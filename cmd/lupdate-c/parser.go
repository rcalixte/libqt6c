	...
	for _, filePath := range filePaths {
		...
		for {
			...
			case token.IDENT:
				...
				if strings.Contains(lit, "_translate") {
					if _, tok, _ := s.Scan(); tok != token.LPAREN {
						continue
					}

					if strings.HasSuffix(lit, "super_translate") || strings.HasSuffix(lit, "_translate4") {
						expectedParams = 4
						expectingNumerus = true

					} else if strings.HasSuffix(lit, "_translate") {
						expectedParams = 2

					} else if strings.HasSuffix(lit, "_translate3") {
						expectedParams = 3
				...
				} else if strings.HasSuffix(lit, "_tr") || strings.HasSuffix(lit, "_tr2") || strings.HasSuffix(lit, "_tr3") {
					name = "QObject"
					...
					} else if strings.HasSuffix(lit, "_tr2") {
						expectedParams = 2

					} else if strings.HasSuffix(lit, "_tr3") {
						expectedParams = 3
						expectingNumerus = true
					}
