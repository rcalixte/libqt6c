#pragma once
#ifndef SRC_EXTRAS_KCONFIG_QT6C_LIBKAUTHORIZED_H
#define SRC_EXTRAS_KCONFIG_QT6C_LIBKAUTHORIZED_H

#include <stdbool.h>
#include <stddef.h>

#include "../libqttypedefs.h"

#pragma GCC diagnostic ignored "-Wdeprecated-declarations"

#include "../qtlibc.h"

/// [Upstream resources](https://api.kde.org/kauthorized.html)

/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#metaObject)
///
/// @param self KAuthorized*
///
const QMetaObject* k_authorized_meta_object(void* self);

/// @param self KAuthorized*
/// @param param1 const char*
///
void* k_authorized_metacast(void* self, const char* param1);

/// @param self KAuthorized*
/// @param param1 enum QMetaObject__Call
/// @param param2 int
/// @param param3 void*
///
int32_t k_authorized_metacall(void* self, int32_t param1, int param2, void* param3);

/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#tr)
///
/// @warning Caller is responsible for freeing the returned memory using `libqt_free()`
///
/// @param s const char*
///
const char* k_authorized_tr(const char* s);

/// [Upstream resources](https://api.kde.org/kauthorized.html#authorize)
///
/// @param action const char*
///
bool k_authorized_authorize(const char* action);

/// [Upstream resources](https://api.kde.org/kauthorized.html#authorize)
///
/// @param action enum KAuthorized__GenericRestriction
///
bool k_authorized_authorize2(int32_t action);

/// [Upstream resources](https://api.kde.org/kauthorized.html#authorizeAction)
///
/// @param action const char*
///
bool k_authorized_authorize_action(const char* action);

/// [Upstream resources](https://api.kde.org/kauthorized.html#authorizeAction)
///
/// @param action enum KAuthorized__GenericAction
///
bool k_authorized_authorize_action2(int32_t action);

/// [Upstream resources](https://api.kde.org/kauthorized.html#authorizeControlModule)
///
/// @param pluginId const char*
///
bool k_authorized_authorize_control_module(const char* pluginId);

/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#tr)
///
/// @warning Caller is responsible for freeing the returned memory using `libqt_free()`
///
/// @param s const char*
/// @param c const char*
///
const char* k_authorized_tr2(const char* s, const char* c);

/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#tr)
///
/// @warning Caller is responsible for freeing the returned memory using `libqt_free()`
///
/// @param s const char*
/// @param c const char*
/// @param n int
///
const char* k_authorized_tr3(const char* s, const char* c, int n);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#event)
///
/// @param self KAuthorized*
/// @param event QEvent*
///
bool k_authorized_event(void* self, void* event);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#eventFilter)
///
/// @param self KAuthorized*
/// @param watched QObject*
/// @param event QEvent*
///
bool k_authorized_event_filter(void* self, void* watched, void* event);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#objectName)
///
/// @warning Caller is responsible for freeing the returned memory using `libqt_free()`
///
/// @param self KAuthorized*
///
const char* k_authorized_object_name(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#setObjectName)
///
/// @param self KAuthorized*
/// @param name const char*
///
void k_authorized_set_object_name(void* self, const char* name);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#isWidgetType)
///
/// @param self KAuthorized*
///
bool k_authorized_is_widget_type(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#isWindowType)
///
/// @param self KAuthorized*
///
bool k_authorized_is_window_type(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#isQuickItemType)
///
/// @param self KAuthorized*
///
bool k_authorized_is_quick_item_type(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#signalsBlocked)
///
/// @param self KAuthorized*
///
bool k_authorized_signals_blocked(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#blockSignals)
///
/// @param self KAuthorized*
/// @param b bool
///
bool k_authorized_block_signals(void* self, bool b);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#thread)
///
/// @param self KAuthorized*
///
QThread* k_authorized_thread(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#moveToThread)
///
/// @param self KAuthorized*
/// @param thread QThread*
///
bool k_authorized_move_to_thread(void* self, void* thread);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#startTimer)
///
/// @param self KAuthorized*
/// @param interval int
///
int32_t k_authorized_start_timer(void* self, int interval);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#startTimer)
///
/// @param self KAuthorized*
/// @param time int64_t of nanoseconds
///
int32_t k_authorized_start_timer2(void* self, int64_t time);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#killTimer)
///
/// @param self KAuthorized*
/// @param id int
///
void k_authorized_kill_timer(void* self, int id);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#killTimer)
///
/// @param self KAuthorized*
/// @param id enum Qt__TimerId
///
void k_authorized_kill_timer2(void* self, int32_t id);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#children)
///
/// @param self KAuthorized*
///
/// @return libqt_list of QObject*
///
libqt_list k_authorized_children(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#setParent)
///
/// @param self KAuthorized*
/// @param parent QObject*
///
void k_authorized_set_parent(void* self, void* parent);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#installEventFilter)
///
/// @param self KAuthorized*
/// @param filterObj QObject*
///
void k_authorized_install_event_filter(void* self, void* filterObj);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#removeEventFilter)
///
/// @param self KAuthorized*
/// @param obj QObject*
///
void k_authorized_remove_event_filter(void* self, void* obj);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#connect)
///
/// @param sender QObject*
/// @param signal const char*
/// @param receiver QObject*
/// @param member const char*
///
QMetaObject__Connection* k_authorized_connect(void* sender, const char* signal, void* receiver, const char* member);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#connect)
///
/// @param sender QObject*
/// @param signal QMetaMethod*
/// @param receiver QObject*
/// @param method QMetaMethod*
///
QMetaObject__Connection* k_authorized_connect2(void* sender, void* signal, void* receiver, void* method);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#connect)
///
/// @param self KAuthorized*
/// @param sender QObject*
/// @param signal const char*
/// @param member const char*
///
QMetaObject__Connection* k_authorized_connect3(void* self, void* sender, const char* signal, const char* member);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param sender QObject*
/// @param signal const char*
/// @param receiver QObject*
/// @param member const char*
///
bool k_authorized_disconnect(void* sender, const char* signal, void* receiver, const char* member);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param sender QObject*
/// @param signal QMetaMethod*
/// @param receiver QObject*
/// @param member QMetaMethod*
///
bool k_authorized_disconnect2(void* sender, void* signal, void* receiver, void* member);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param self KAuthorized*
///
bool k_authorized_disconnect3(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param self KAuthorized*
/// @param receiver QObject*
///
bool k_authorized_disconnect4(void* self, void* receiver);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param param1 QMetaObject__Connection*
///
bool k_authorized_disconnect5(void* param1);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#dumpObjectTree)
///
/// @param self KAuthorized*
///
void k_authorized_dump_object_tree(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#dumpObjectInfo)
///
/// @param self KAuthorized*
///
void k_authorized_dump_object_info(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#setProperty)
///
/// @param self KAuthorized*
/// @param name const char*
/// @param value QVariant*
///
bool k_authorized_set_property(void* self, const char* name, void* value);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#property)
///
/// @param self KAuthorized*
/// @param name const char*
///
QVariant* k_authorized_property(void* self, const char* name);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#dynamicPropertyNames)
///
/// @warning Caller is responsible for freeing the returned memory using `free()`
///
/// @param self KAuthorized*
///
const char** k_authorized_dynamic_property_names(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#bindingStorage)
///
/// @param self KAuthorized*
///
QBindingStorage* k_authorized_binding_storage(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#bindingStorage)
///
/// @param self KAuthorized*
///
const QBindingStorage* k_authorized_binding_storage2(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#destroyed)
///
/// @param self KAuthorized*
///
void k_authorized_destroyed(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#destroyed)
///
/// @param self KAuthorized*
/// @param callback void func(KAuthorized* self)
///
void k_authorized_on_destroyed(void* self, void (*callback)(void*));

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#parent)
///
/// @param self KAuthorized*
///
QObject* k_authorized_parent(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#inherits)
///
/// @param self KAuthorized*
/// @param classname const char*
///
bool k_authorized_inherits(void* self, const char* classname);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#deleteLater)
///
/// @param self KAuthorized*
///
void k_authorized_delete_later(void* self);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#startTimer)
///
/// @param self KAuthorized*
/// @param interval int
/// @param timerType enum Qt__TimerType
///
int32_t k_authorized_start_timer22(void* self, int interval, int32_t timerType);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#startTimer)
///
/// @param self KAuthorized*
/// @param time int64_t of nanoseconds
/// @param timerType enum Qt__TimerType
///
int32_t k_authorized_start_timer23(void* self, int64_t time, int32_t timerType);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#connect)
///
/// @param sender QObject*
/// @param signal const char*
/// @param receiver QObject*
/// @param member const char*
/// @param param5 enum Qt__ConnectionType
///
QMetaObject__Connection* k_authorized_connect5(void* sender, const char* signal, void* receiver, const char* member, int32_t param5);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#connect)
///
/// @param sender QObject*
/// @param signal QMetaMethod*
/// @param receiver QObject*
/// @param method QMetaMethod*
/// @param type enum Qt__ConnectionType
///
QMetaObject__Connection* k_authorized_connect52(void* sender, void* signal, void* receiver, void* method, int32_t type);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#connect)
///
/// @param self KAuthorized*
/// @param sender QObject*
/// @param signal const char*
/// @param member const char*
/// @param type enum Qt__ConnectionType
///
QMetaObject__Connection* k_authorized_connect4(void* self, void* sender, const char* signal, const char* member, int32_t type);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param self KAuthorized*
/// @param signal const char*
///
bool k_authorized_disconnect1(void* self, const char* signal);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param self KAuthorized*
/// @param signal const char*
/// @param receiver QObject*
///
bool k_authorized_disconnect22(void* self, const char* signal, void* receiver);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param self KAuthorized*
/// @param signal const char*
/// @param receiver QObject*
/// @param member const char*
///
bool k_authorized_disconnect32(void* self, const char* signal, void* receiver, const char* member);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#disconnect)
///
/// @param self KAuthorized*
/// @param receiver QObject*
/// @param member const char*
///
bool k_authorized_disconnect23(void* self, void* receiver, const char* member);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#destroyed)
///
/// @param self KAuthorized*
/// @param param1 QObject*
///
void k_authorized_destroyed1(void* self, void* param1);

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#destroyed)
///
/// @param self KAuthorized*
/// @param callback void func(KAuthorized* self, QObject* param1)
///
void k_authorized_on_destroyed1(void* self, void (*callback)(void*, void*));

/// Inherited from QObject
///
/// [Upstream resources](https://doc.qt.io/qt-6/qobject.html#objectNameChanged)
///
/// Wrapper to allow calling private signal
///
/// @param self KAuthorized*
/// @param callback void func(KAuthorized* self, const char* objectName)
///
void k_authorized_on_object_name_changed(void* self, void (*callback)(void*, const char*));

/// [Upstream resources](https://api.kde.org/kauthorized.html#dtor.KAuthorized)
///
/// Delete this object from C++ memory.
///
/// @param self KAuthorized*
///
void k_authorized_delete(void* self);

/// [Upstream resources](https://api.kde.org/kauthorized.html#public-types)

typedef enum {
    KAUTHORIZED_GENERICRESTRICTION_SHELL_ACCESS = 1,
    KAUTHORIZED_GENERICRESTRICTION_GHNS = 2,
    KAUTHORIZED_GENERICRESTRICTION_LINEEDIT_REVEAL_PASSWORD = 3,
    KAUTHORIZED_GENERICRESTRICTION_LINEEDIT_TEXT_COMPLETION = 4,
    KAUTHORIZED_GENERICRESTRICTION_MOVABLE_TOOLBARS = 5,
    KAUTHORIZED_GENERICRESTRICTION_RUN_DESKTOP_FILES = 6
} KAuthorized__GenericRestriction;

/// [Upstream resources](https://api.kde.org/kauthorized.html#public-types)

typedef enum {
    KAUTHORIZED_GENERICACTION_OPEN_WITH = 1,
    KAUTHORIZED_GENERICACTION_EDITFILETYPE = 2,
    KAUTHORIZED_GENERICACTION_OPTIONS_SHOW_TOOLBAR = 3,
    KAUTHORIZED_GENERICACTION_SWITCH_APPLICATION_LANGUAGE = 4,
    KAUTHORIZED_GENERICACTION_BOOKMARKS = 5
} KAuthorized__GenericAction;

#endif
